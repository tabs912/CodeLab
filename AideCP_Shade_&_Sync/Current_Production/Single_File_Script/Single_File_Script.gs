...
const GrayShade = (function() {
  ...
  function normalizeLayoutLabel_(value) {
    return String(value || '')
      .trim()
      .replace(/^[-\s]+/, '')
      .replace(/:\s*$/, '')
  ...
  function verifyLayoutStructure_(sheet) {
    const matrix = {
      ...
      "H13": ["Laundry"],
      "H15": ["Shopping"],
      "N18": ["Change PAP Filter/Tubing", "Change O2 tubing/humidifier"]
    };
    for (const [cell, expectedTexts] of Object.entries(matrix)) {
      const actualVal = normalizeLayoutLabel_(sheet.getRange(cell).getValue());
      const matches = expectedTexts.some(expectedText => actualVal === normalizeLayoutLabel_(expectedText));
...
function onOpen() {
  ...
  const maintMenu = ui.createMenu('🛠️ Maintenance')
    ...
    .addItem('Validate Layout/Mapping (Current Sheet)', 'validateMappingsCurrentSheet')
    .addItem('Validate Layout/Mapping (All Eligible Sheets)', 'validateMappingsAllSheets')
