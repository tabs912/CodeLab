...
**Current reviewed script:** `Master_List/Current Production Script/v1.9.7.4.6`
**Current evidence:** timing and Dashboard Quality reports in `Master_List/Audit Summary/v1.9.7.4.6/`
...
- [ ] Consolidate `isPrimaryPMRRowValue_` to one declaration.
- [ ] Consolidate `writeUniqueParticipantTotalToG1_` and retain the approved rich-text totals behavior.
- [ ] Consolidate `hideReportTemplates_` to one declaration.
- [ ] Consolidate `runDashboardQualityValidateTemplates` to one declaration and one row schema.
- [ ] Remove the duplicate `setupArchiveSyncTrigger()` call from `quickSystemSetup()`.
- [ ] Leave exactly one owned 15-minute archive trigger after setup.
- [ ] Use one guarded lock release per workflow; keep quality outside the lock.
- [ ] Confirm zero duplicate top-level function names and all menu/trigger callbacks resolve.
...
- [ ] Inject failures before/after every clear, write, rename, delete, and flush.
- [ ] Prove one valid governed target survives every factory failure boundary.
- [ ] Prove dual-sheet operations either fully commit or fully restore.
- [ ] Test zero rows, mixed changes, re-enrollment, and idempotent repeat.
...
- [ ] Batch operational subheader values and formatting.
- [ ] Cache dashboard/theme/configuration inputs reused by template builders.
- [ ] Eliminate avoidable range/service calls inside template row loops.
...
- [ ] Short-circuit verification when no source-only columns require preservation.
- [ ] Reuse already-read matrices and avoid redundant full-grid comparisons.
- [ ] Profile and batch Index refresh calls.
...
- [ ] Decide whether the legacy image-button path is retired or supported.
- [ ] Before removal, check menus, triggers, assigned scripts, `onEdit`, strings, and external consumers.
- [ ] Runtime-test checkbox selection, divider rows, cancellation, success, archive failure, and repeat sync.
- [ ] Retain one documented, unambiguous restore interface.
...
- [ ] Run Framework Smoke Validation and all focused failure cases.
