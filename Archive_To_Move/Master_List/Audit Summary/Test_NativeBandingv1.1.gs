/**
 * Test_NativeBanding.gs
 *
 * Isolated native-banding test harness with data-processing parity to production
 * v1.8.9.7. All copied implementation symbols are enclosed in NativeBandingTest
 * so this file can be added beside production without overriding production APIs.
 *
 * Test archive: 1i9gekRO0zkiGC6hLlwt8z2P1najoaIHl7yOXWULp_RI
 */
const NativeBandingTest = (function() {
"use strict";
...
const MASTER_LIST_MERGE_ML_VERSION = "1.8.9.7-NATIVE-BANDING-TEST";
...
const TEST_RFF_ARCHIVE_SPREADSHEET_ID = "1i9gekRO0zkiGC6hLlwt8z2P1najoaIHl7yOXWULp_RI";

const RFF_ARCHIVE_SPREADSHEET_ID = TEST_RFF_ARCHIVE_SPREADSHEET_ID;

function getArchiveSpreadsheetId_() {
  const props = PropertiesService.getDocumentProperties();
  const storedId = props.getProperty("TEST_RFF_ARCHIVE_SPREADSHEET_ID");
...
function configureArchiveSpreadsheetId() {
  ...
  PropertiesService.getDocumentProperties().setProperty("TEST_RFF_ARCHIVE_SPREADSHEET_ID", newId);
...
const MASTER_LIST_PREFIX = "TEST Master List";

const DEMO_P_PREFIX = "TEST Demo P";

const BANNER_PREFIX = "TEST Banners";
...
const MONTHLY_CHANGE_REPORT_PREFIX = "TEST Monthly Change";

const DISENROLLED_EXCLUSION_SHEET = "TEST Disenrolled Exclusion";

const DEMO_P_ARCHIVE_SHEET = "Archive - Demo P";
...
const MASTER_LIST_TEMPLATE_SHEET = "TEST Template - Master List";

const DEMO_P_TEMPLATE_SHEET = "TEST Template - Demo P";
...
function getFormatDashboardExpectedSectionHeaders_() {
  ...
  map[RFF_SECTION_SYSTEM_SURFACES] = normalizeSectionRowForWidth_(["System Sheet Name", "Display Name", "Sort Order", "Output Visibility", "Title Fill Color", "Title Font Color", "Use Global Defaults", "Notes"], width);
...
function getDefaultSheetDefinitionByType_(sheetType) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    return {
      ...
      templateName: ensureTestTemplateName_(String(row[2] || ("TEST Template - " + row[0])).trim()),
...
function createTemplateFromScriptDefaultsForActiveBuild_(ss, sheetDef) {
  ...
  applyNativeBanding_(template, template.getRange(DATA_START_ROW, 1, Math.max(rowCount - DATA_START_ROW + 1, 1), width), theme);
...
function isTemplateSheetName_(sheetName) {
  return String(sheetName || "").trim().indexOf("TEST Template - ") === 0;
}
...
function assertActiveWorkflowSourceSheet_(sheet, workflowName, options) {
  ...
  if (getActiveWorkflowBlockedSheetNames_().has(sheetName) || /^RFF_/i.test(sheetName) || isTemplateSheetName_(sheetName)) {
    throw new Error("The active sheet '" + sheetName + "' is not a valid source/output sheet for " + workflowName + ". Select the imported or formatted active sheet, not a dashboard, index, timing, archive, system, or template sheet.");
  }
...
function assertActiveRawDataSourceSheet_(sheet, workflowName) {
  ...
  if (getPMRIndex_(headerMap) === -1) {
    throw new Error("The active sheet '" + sheet.getName() + "' does not look like a Raw Data import/source sheet because no Participant PMR# / PMR # header was found on row 1 or row " + HEADER_ROW + ".");
  }
  return sheet;
}
...
function preflightMonthlyUpdateForMonth_(monthParts, timing) {
  ...
  if (existingMasterListSheet) {
    ...
    if (response !== ui.Button.YES) {
      throw new Error("Monthly Update cancelled: Existing Master List not replaced.");
    }
...
function quickSystemSetup() {
  ...
  createIndexSheet({ forceShellRebuild: true });
...
function quickBuildAllTemplates() {
  showQuickStartToast_("Quick Build All Templates: creating/refreshing templates in place...");
  createOrRefreshAllReportTemplates();
  showQuickStartToast_("Quick Build All Templates: validating templates...");
...
function getMonthlySheetByPrefixAndDate_(ss, prefix, firstDay, lastDay) {
  ...
  if (cachedSheet) {
    try {
      ...
      delete getRuntimeCache_().monthlySheets[cacheKey];
    }
  }
...
function getHeaders_(sheet, headerRow) {
  ...
  if (key && getRuntimeCache_().headers[key]) {
    return getRuntimeCache_().headers[key].slice();
  }
...
function getHeaderMap_(sheet, headerRow) {
  ...
  if (key && getRuntimeCache_().headerMaps[key]) {
    return Object.assign({}, getRuntimeCache_().headerMaps[key]);
  }
...
function validateRawDataPreflightForDemoP_(rawSheet, rawData, workflowName) {
  ...
  if (!rawSheet) {
    throw new Error(label + " stopped: Raw Data sheet was not found. Format Raw Data first.");
  }
  ...
  if (pmrIdx === -1) {
    throw new Error(
      label + " stopped: Raw Data is missing a Participant PMR header. " +
      "Accepted headers: Participant PMR#, PMR #, PMR#, Participant PMR. " +
      "Headers found: " + (nonBlankHeaders.length ? nonBlankHeaders.join(", ") : "(none)") + "."
    );
  }
...
function getDataValues_(sheet, headerRow, dataStartRow) {
  ...
  // OPTIMIZATION: Restrict rows read to the explicit header boundary column count to eliminate empty cell processing
...
function trimOutputSheetToDataSize_(sheet, rowCount, sheetType) {
  ...
  const templateRows = resolveTemplateRowCount_(sheetDef || { templateRowMode: "FIXED" }, RFF_DEFAULTS || {});
...
function clearHeaderCacheForSheet_(sheet) {
  ...
  Object.keys(getRuntimeCache_().headers).forEach(key => {
    if (key.indexOf(prefix) === 0) delete getRuntimeCache_().headers[key];
  });


  Object.keys(getRuntimeCache_().headerMaps).forEach(key => {
    if (key.indexOf(prefix) === 0) delete getRuntimeCache_().headerMaps[key];
  });
}
...
function getSheetDimensions_(sheet) {
  const key = getSheetDimensionCacheKey_(sheet);
  if (key && getRuntimeCache_().dimensions[key]) {
    return Object.assign({}, getRuntimeCache_().dimensions[key]);
  }
...
function writeCombinedFrameworkTimingReport_() {
  ...
  const options = { skipStyle: true, skipShellCheck: true };
  ...
  // Style sections A, B, and C only (leaves Section D milestone highlighting intact)
...
function rebuildFrameworkTimingReportShellCompact_(sheet) {
  ...
  const rows = [];
  rows.push(normalizeSectionRowForWidth_(["TEST - Timing Report", "- v" + MASTER_LIST_MERGE_ML_VERSION + " -", "Report Timing Framework"], requiredCols));
...
function hasFrameworkTimingReportShell_(sheet) {
  ...
  if (titleValues[0] !== "TEST - Timing Report") return false;
  ...
  for (let i = 0; i < sections.length; i++) {
    if (!existingTitles.has(sections[i].title)) return false;
  }
...
function initializeFrameworkTimingSheet_() {
  ...
  if (!sheet) {
    sheet = insertGovernedOutputSheet_(ss, getFrameworkTimingReportSheetName_());
  } else {
...
function styleFrameworkTimingReport_(sheet, sectionIds) {
  ...
  const widths = [220, 180, 475, 140, 140, 260, 120, 120];
  trimSheetToColumnCount_(sheet, widths.length, "TEST - Timing Report");
  ...
  try {
    ranges.forEach(function(item) {
      ...
      if (item.startRow <= 3) {
        sheet.getRange(item.startRow, 1, rowCount, width).setWrapStrategy(SpreadsheetApp.WrapStrategy.OVERFLOW);
      } else {
        sheet.getRange(item.startRow, 1, rowCount, 1).setWrapStrategy(SpreadsheetApp.WrapStrategy.OVERFLOW);
        sheet.getRange(item.startRow, 2, rowCount, width - 1).setWrapStrategy(SpreadsheetApp.WrapStrategy.WRAP);
      }
    });
...
function replaceFrameworkTimingSectionRows_(sheet, sectionId, rows, options) {
  ...
  if (!(options && options.skipShellCheck) && !hasFrameworkTimingReportShell_(sheet)) {
    rebuildFrameworkTimingReportShellCompact_(sheet);
  }
...
function appendRuntimeTimingToFrameworkTimingReport_(timing) {
  ...
  try {
    ...
    // RE-CALCULATE TRUE FINAL MILESTONE ROW AFTER INSERTS
 
    const trueLastRow = sheet.getLastRow();
    sheet.getRange(trueLastRow, 1, 1, 7)
      .setFontWeight("bold")
...
function installMenu_() {
  const ui = SpreadsheetApp.getUi();
  const templates = ui.createMenu("Templates")
    .addItem("Build Test Templates (Native Banding)", "buildTestTemplatesNativeBanding")
    .addItem("Show TEST Templates", "showTestTemplates")
    .addItem("Hide TEST Templates", "hideTestTemplates");
  const subReports = ui.createMenu("Monthly Sub-Reports")
    .addItem("Show TEST Monthly Sub-Reports", "showTestMonthlySubReports")
    .addItem("Hide TEST Monthly Sub-Reports", "hideTestMonthlySubReports");
  const activeReports = ui.createMenu("Monthly Active Reports")
    .addItem("Show TEST Monthly Active Reports", "showTestMonthlyActiveReports")
    .addItem("Hide TEST Monthly Active Reports", "hideTestMonthlyActiveReports");
  ui.createMenu("🧪 TEST: Native Banding")
    .addSubMenu(templates)
    .addItem("Format Monthly Sheets", "testFormatMonthlySheets")
    .addItem("Create Monthly Start", "testCreateMonthlyStart")
    .addItem("Create Monthly Update", "testCreateMonthlyUpdate")
    .addSubMenu(subReports)
    .addSubMenu(activeReports)
    .addItem("Hide TEST Source Sheets", "hideTestSourceSheets")
    .addToUi();
}
...
function setVisibilityByPredicate_(predicate, visible) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const active = ss.getActiveSheet();
  ss.getSheets().forEach(function(sheet) {
    if (!predicate(sheet.getName())) return;
    if (visible) {
      sheet.showSheet();
    } else if (sheet.getSheetId() !== active.getSheetId()) {
      sheet.hideSheet();
    }
  });
}

function showTestTemplates_() {
  setVisibilityByPredicate_(function(name) { return name.indexOf("TEST Template - ") === 0; }, true);
}

function hideTestTemplates_() {
  setVisibilityByPredicate_(function(name) { return name.indexOf("TEST Template - ") === 0; }, false);
}

function isTestMonthlySubReport_(name) {
  return /^TEST (Banners|CP Due|Unlock CP|Unlocked CP)/.test(name);
}

function showTestMonthlySubReports_() {
  setVisibilityByPredicate_(isTestMonthlySubReport_, true);
}

function hideTestMonthlySubReports_() {
  setVisibilityByPredicate_(isTestMonthlySubReport_, false);
}

function isTestMonthlyActiveReport_(name) {
  return /^TEST (Master List|Demo P|Monthly Change|Disenrolled Exclusion)/.test(name);
}

function showTestMonthlyActiveReports_() {
  setVisibilityByPredicate_(isTestMonthlyActiveReport_, true);
}

function hideTestMonthlyActiveReports_() {
  setVisibilityByPredicate_(isTestMonthlyActiveReport_, false);
}

function hideTestSourceSheets_() {
  setVisibilityByPredicate_(function(name) {
    return /^TEST Raw Data/.test(name) || /^TEST Source/.test(name);
  }, false);
}
...
function getDefaultSheetDefinitionRows_() {
  return [
  ["Banners", "Banner Report", "TEST Template - Banner Report", "Banners mm.yy", "#65A9CC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25],
  ["CP Due Date", "Care Plan Due Date Report", "TEST Template - Care Plan Due", "CP Due mm.yy", "#65CC99", true, "Pulled From Spreadsheet", 100, "FIXED", 100, 25],
  ["Unlock CP", "Unlocked Care Plan Report", "TEST Template - Unlocked Care Plan", "Unlock CP mm.yy", "#65CCC3", true, "Pulled From Spreadsheet", 100, "FIXED", 100, 25],
  ["Raw Data", "Raw Data", "TEST Template - Raw Data", "Raw Data mm.yy", "#657FCC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25],
  ["Demo P", "Demo P", "TEST Template - Demo P", "Demo P", "#657FCC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25],
  ["Disenrolled Exclusion", "Disenrolled Exclusion", "TEST Template - Disenrolled Exclusion", "Disenrolled", "#CC65A1", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25],
  ["Master List", "Master List", "TEST Template - Master List", "Master List mm.yy", "#7665CC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25],
  ["Monthly Change", "Monthly Change Report", "TEST Template - Monthly Change", "Monthly Change mm.yy", "#A165CC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25]
];
}
...
function getDefaultTabOrganizationRows_() {
  return [
    ...
    ["TEST - Timing Report", "System & Configuration", "500", ""],
    ["TEST - Quality Report", "System & Configuration", "501", ""],
    ["Format Dashboard", "System & Configuration", "502", ""],
    ["TEST Template - Banner Report", "Template", "801", ""],
    ["TEST Template - Care Plan Due", "Template", "802", ""],
    ["TEST Template - Unlocked Care Plan", "Template", "803", ""],
    ["TEST Template - Raw Data", "Template", "804", ""],
    ["TEST Template - Demo P", "Template", "805", ""],
    ["TEST Template - Disenrolled Exclusion", "Template", "806", ""],
    ["TEST Template - Master List", "Template", "807", ""],
    ["TEST Template - Monthly Change", "Template", "808", ""],
...
function getDefaultSystemSurfaceRows_() {
  return [
    ["TEST - Timing Report", "TEST - Timing Report", 500, "VISIBLE", RFF_SYSTEM_SHEET_TITLE_COLOR, "#000000", true, "Unified timing report surface"],
    ["TEST - Quality Report", "TEST - Quality Report", 501, "VISIBLE", RFF_SYSTEM_SHEET_TITLE_COLOR, "#000000", true, "Unified quality report surface"],
...
function createOrRefreshAllReportTemplates() {
  return runFrameworkTimed_("Create / Refresh All Templates", function(timing) {
    ...
    ensureRequiredMasterListTemplate_(dashboard, timing);
    setReportTemplateVisibility_(dashboard, true, timing);
    forceBaseTemplateHidden_();
    markFrameworkStep_(timing, "Templates hidden once after template creation");
    createIndexSheet();
    markFrameworkStep_(timing, "Index refreshed after template creation without showing hidden sheets");
...
function setReportTemplateVisibility_(dashboard, hidden, timing) {
  ...
  Object.keys(sheetMap).forEach(function(sheetName) {
    if (sheetName === RFF_BASE_TEMPLATE_NAME) return;
    if (String(sheetName || "").indexOf("TEST Template - ") !== 0) return;
...
function ensureTestTemplateName_(templateName) {
  const name = String(templateName || "").trim();
  if (name.indexOf("TEST Template - ") === 0) return name;
  if (name.indexOf("Template - ") === 0) return "TEST " + name;
...
function promoteStagedTemplateBuild_(ss, buildSheet, existingSheet, sheetDef, timing) {
  if (existingSheet && existingSheet.getSheetId() !== buildSheet.getSheetId()) {
    deleteSheetSafely_(ss, existingSheet, "staged template promotion", [buildSheet.getName()]);
  ...
  placeCreatedSheetInConfiguredOrder_(buildSheet);
  if (!RFF_DEFER_TEMPLATE_HIDE_DURING_BATCH_ && typeof buildSheet.hideSheet === "function") {
...
function buildTemplateFromDashboard_(sheet, dashboard, sheetDef, headers, rowCount, colCount, behavior, expectedSignature, timing, templateExisted) {
  ...
  if (behavior.hiddenTemplate) {
    if (RFF_DEFER_TEMPLATE_HIDE_DURING_BATCH_) {
      markFrameworkStep_(timing, "Template hide deferred until batch completion: " + sheetDef.templateName);
    } else {
...
function lockFinalOutputRowHeights_(sheet, label) {
  ...
  try {
    ...
    if (lastRow >= dataStartRow && shouldLockExpandedDataRowHeights_(sheet, label)) {
...
function shouldLockExpandedDataRowHeights_(sheet, label) {
  ...
  return name.indexOf(DEMO_P_PREFIX) === 0 ||
    ...
    name === DISENROLLED_EXCLUSION_SHEET ||
...
// ----------------------------------------------------------------------------
// TEMPLATE GENERATOR ENGINE (NATIVE BANDING + DASHBOARD LEVEL 3/4 COLORS)
// ----------------------------------------------------------------------------

// ============================================================================
// TEMPLATE BUILD & FORMATTING ENGINE (PURE RE-ORDER; NO BREAK APART)
// ============================================================================

// ============================================================================
// TEMPLATE BUILD & FORMATTING ENGINE (COMPLETE BLOCK - NO COLUMN HIDING)
// ============================================================================
...
function applyTemplateBaseFormatting_(sheet, dashboard, sheetDef, headers, rowCount, colCount, behavior, timing) {
  ...
  const headerRow = globals.headerRow || HEADER_ROW;
  const dataStartRow = globals.dataStartRow || DATA_START_ROW;
  const rows = Math.max(rowCount - dataStartRow + 1, 1);

  // 1. Apply universal canvas base formatting across all cells
  ...
  // 2. STEP 1: APPLY NATIVE BANDING FIRST (Canvas is completely fresh & unmerged grid)
  if (behavior.usesAlternatingColors) {
    const dataRange = sheet.getRange(dataStartRow, 1, rows, colCount);
    applyNativeBanding_(sheet, dataRange, theme);
    if (timing) markFrameworkStep_(timing, "Apply native banding to template: " + sheetDef.templateName);
  } else {
    if (timing) markFrameworkStep_(timing, "Skip alternating color rules: " + sheetDef.templateName);
  }

  // 3. STEP 2: APPLY TITLE ROWS & EXECUTE MERGES (Runs AFTER native banding is bound)
  try {
    applyTitleRows_(sheet, dashboard, sheetDef, theme, colCount);
    if (timing) markFrameworkStep_(timing, "Apply title rows and merges: " + sheetDef.templateName);
  } catch (mergeErr) {
    if (timing) markFrameworkStep_(timing, "ERROR - Title Row Merge Conflict: " + mergeErr.message);
    logBestEffortWarning_("Title row merge warning on " + sheet.getName() + ": " + mergeErr.message);
  }

  // 4. STEP 3: APPLY TABLE HEADERS & COLUMN FORMATTING
  applyHeaderRow_(sheet, dashboard, sheetDef, headers, theme, colCount);
  if (timing) markFrameworkStep_(timing, "Apply header row: " + sheetDef.templateName);

  applyColumnWidths_(sheet, dashboard, headers);
  if (timing) markFrameworkStep_(timing, "Apply column widths: " + sheetDef.templateName);

  applyGovernedTextAndNumberFormats_(sheet, dashboard, headers, dataStartRow, rows);
  if (timing) markFrameworkStep_(timing, "Apply governed text/date/number formats: " + sheetDef.templateName);

  // Exclude column hiding during template creation to prevent UI preview/overlap issues
  applyHiddenColumnSettings_(sheet, dashboard, headers);
  if (timing) markFrameworkStep_(timing, "Enforce all columns visible: " + sheetDef.templateName);
...
function applyTitleRows_(sheet, dashboard, sheetDef, theme, colCount) {
  ...
  try {
    ...
    if (timing) markFrameworkStep_(timing, "ERROR - Title Info Merge Collision", err.message);
...
function rowColToA1_(row, col) {
  let column = Math.max(Number(col || 1), 1);
  let letters = "";
  while (column > 0) {
  ...
  return letters + Math.max(Number(row || 1), 1);
}
  ...
  Object.keys(rangesByFormat).forEach(function(googleSheetsFormat) {
    ...
    try {
      if (typeof sheet.getRangeList !== "function") {
        throw new Error("RangeList API is unavailable");
      }
...
/**
 * Ensures all columns remain 100% visible on templates and output sheets to prevent
 * range/merge preview collisions.
 */
function applyHiddenColumnSettings_(sheet, dashboard, headers) {
  if (!sheet) return;
  try {
    const maxCols = Math.max(sheet.getMaxColumns(), (headers || []).length, 1);
    sheet.showColumns(1, maxCols);
  } catch (err) {
    logBestEffortWarning_("Unhide columns skipped for " + sheet.getName() + ": " + err.message);
  }
}
...
function applyHiddenColumnSettingsInRuns_(sheet, hiddenFlags) {
  if (!sheet) return;
  try {
    const maxCols = Math.max(sheet.getMaxColumns(), (hiddenFlags || []).length, 1);
    sheet.showColumns(1, maxCols);
  } catch (err) {
    logBestEffortWarning_("Unhide column run skipped for " + sheet.getName() + ": " + err.message);
  }
}
...
function applyDataRows_(sheet, dashboard, sheetDef, headers, rowCount, colCount, behavior, timing) {
  ...
  if (behavior.usesAlternatingColors) {
    applyNativeBanding_(sheet, dataRange, theme);
    if (timing) markFrameworkStep_(timing, "Apply alternating color rules: " + sheetDef.templateName);
  } else {
    if (timing) markFrameworkStep_(timing, "Skip alternating color rules: " + sheetDef.templateName);
  }
}

function applyNativeBanding_(sheet, range, theme) {
  if (!sheet || !range) return null;

  const rows = Math.max(range.getNumRows(), 1);
  const cols = Math.max(range.getNumColumns(), 1);
  const startRow = range.getRow();
  const startColumn = range.getColumn();
  const oddColor = (theme && theme.level3) || "#FFFFFF";
  const evenColor = (theme && theme.level4) || "#F7F7F7";

  const target = sheet.getRange(startRow, startColumn, rows, cols);
  const targetLastRow = target.getLastRow();
  const targetLastColumn = target.getLastColumn();

  // Safely remove existing bandings on the target data area only
  sheet.getBandings().forEach(function(banding) {
    const bandedRange = banding.getRange();
    const overlaps = bandedRange.getRow() <= targetLastRow &&
      bandedRange.getLastRow() >= startRow &&
      bandedRange.getColumn() <= targetLastColumn &&
      bandedRange.getLastColumn() >= startColumn;
    if (overlaps) banding.remove();
  });

  const banding = target.applyRowBanding(SpreadsheetApp.BandingTheme.LIGHT_GREY, false, false);
  banding.setFirstRowColor(oddColor);
  banding.setSecondRowColor(evenColor);
  return banding;
}

function applyAlternatingColorsForSheetType_(sheet, dashboard, sheetType, rowCount, width, timing, label) {
  if (!sheet) return;
  const effectiveDashboard = dashboard || loadDashboardConfig_();
  const behavior = getBehaviorForSheetType_(effectiveDashboard, sheetType);
  if (!behavior.usesAlternatingColors) return;

  const safeWidth = Math.max(Number(width || sheet.getLastColumn() || 1), 1);
  const lastDataRow = Math.max(Number(rowCount || sheet.getLastRow() || DATA_START_ROW), DATA_START_ROW);
  const dataRows = Math.max(lastDataRow - DATA_START_ROW + 1, 1);
  const sheetDef = getSheetDefinitionByTypeOrNull_(effectiveDashboard, sheetType) || getDefaultSheetDefinitionByType_(sheetType);
  const theme = getThemeColorsFromBase_(sheetDef.baseColor, effectiveDashboard.globals || RFF_DEFAULTS);

  applyNativeBanding_(sheet, sheet.getRange(DATA_START_ROW, 1, dataRows, safeWidth), theme);
  if (timing && label) markFrameworkStep_(timing, label);
}
...
function formatMonthlyChangeSubsectionBlock_(sheet, titleRow, label, colCount, dashboard, sheetDef, writeLabel) {
  ...
  // ROW 1: SECTION TITLE BAR
  const titleRange = sheet.getRange(titleRow, 1, 1, colCount);
  titleRange
    .merge()
    .setBackground(sheetDef.level5Fill || theme.level2)
    .setFontFamily(globals.standardFont)
    .setFontSize(Number(globals.titleFontSize || 12))
    .setFontWeight("bold")
    .setFontColor("#FFFFFF")
    .setWrapStrategy(SpreadsheetApp.WrapStrategy.WRAP)
    .setHorizontalAlignment("left")
  ...
  // ROW 2: BLANK DIVIDER / SPACER ROW
  const spacerRange = sheet.getRange(spacerRow, 1, 1, colCount);
  spacerRange
    .clearContent()
    .setBackground("#FFFFFF");
  sheet.setRowHeight(spacerRow, Number(globals.row3Height || 10));

  // ROW 3: SECTION COLUMN HEADERS
  const headerRange = sheet.getRange(headerRow, 1, 1, colCount);
  headerRange
    .setBackground(sheetDef.level2Fill || theme.level2)
    .setFontFamily(globals.standardFont)
    ...
    .setHorizontalAlignment("left")
    .setVerticalAlignment("top");
  sheet.setRowHeight(headerRow, Number(globals.headerRowHeight || 35));
  ...
  if (writeLabel) {
    ...
    if (monthlyHeaders.length > 0) {
      headerRange.setValues([monthlyHeaders]);
    }
  }
}
...
function getDefaultBehaviorForSheetType_(sheetType) {
  const normalized = normalizeDashboardSheetTypeKey_(sheetType);
  const rows = getDefaultBehaviorRows_();
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    if (normalizeDashboardSheetTypeKey_(row[0]) !== normalized) continue;
    return {
...
function activateVisibleSheetBeforeHiding_(sheet) {
  ...
  const fallback = ss.getSheets().filter(function(candidate) {
    return candidate.getSheetId() !== sheet.getSheetId() &&
      (!candidate.isSheetHidden || !candidate.isSheetHidden());
  })[0];
...
function hideSheetIfNeeded_(sheet, timing, stepName) {
  ...
  try {
    if (typeof sheet.isSheetHidden === "function" && !sheet.isSheetHidden()) {
      if (!activateVisibleSheetBeforeHiding_(sheet)) {
        throw new Error("No visible fallback sheet is available before hiding the active sheet.");
      }
      sheet.hideSheet();
      if (timing && stepName) markFrameworkStep_(timing, stepName);
    }
...
// ----------------------------------------------------------------------------
//  FORMAT MONTHLY SHEETS (100% PRODUCTION PARITY & IN-PLACE RAW DATA)
// ----------------------------------------------------------------------------
...
function formatMonthlySheets() {
  ...
  return runFrameworkTimed_("Format Monthly Sheets " + parsed.monthLabel, function(timing) {
    ...
    if (RFF_ENABLE_AUTO_ARCHIVE_RAW_DATA) {
      try {
        const archiveId = typeof getDocumentPropertiesCached_ === "function" ?
          (getDocumentPropertiesCached_().getProperty("TEST_RFF_ARCHIVE_SPREADSHEET_ID") || RFF_ARCHIVE_SPREADSHEET_ID) :
          (PropertiesService.getDocumentProperties().getProperty("TEST_RFF_ARCHIVE_SPREADSHEET_ID") || RFF_ARCHIVE_SPREADSHEET_ID);
    ...
    const routes = [
      { code: "B", label: "Banners", fn: function(sheet) { return formatMonthlyBannerSheet_(sheet, parsed, timing, sharedOptions); } },
      { code: "CD", label: "Care Plan Due", fn: function(sheet) { return formatMonthlyDashboardSheetFromSource_(sheet, RFF_SHEET_TYPES.CARE_PLAN_DUE, parsed.monthParts, "C3", "E3", timing, sharedOptions); } },
      { code: "UC", label: "Unlocked CP", fn: function(sheet) { return formatMonthlyDashboardSheetFromSource_(sheet, RFF_SHEET_TYPES.UNLOCKED, parsed.monthParts, "E2", "G2", timing, sharedOptions); } },
      { code: "RD", label: "Raw Data", fn: function(sheet) { return formatMonthlyRawDataSheetFromSource_(sheet, parsed.monthParts, timing, sharedOptions); } }
    ];
    ...
    try {
      // WAVE 4 OPTIMIZATION: Pass pre-opened archive handle to prevent re-opening connection
      createIndexSheet(archiveSs);
...
function sheetNameMatchesPromptedMonth_(sheetName, monthParts) {
  ...
  const variants = [
    ...
    String(month) + " " + year2,
    (month < 10 ? "0" + month : String(month)) + " " + year2,
    String(month) + "." + year2,
    (month < 10 ? "0" + month : String(month)) + "." + year2
  ];
...
function resolveMonthlyFormatterSourceSheet_(ss, activeSheet, monthParts, keywords, validator, workflowName, preferredOutputName) {
  const monthLabel = Utilities.formatDate(monthParts.firstDay, Session.getScriptTimeZone(), "MM.yy");
  const candidates = [];
  if (activeSheet) candidates.push(activeSheet);

  findArchiveMonthlyCandidateSheets_(ss, keywords, monthLabel).forEach(function(sheet) {
    if (!candidates.some(function(candidate) { return candidate.getSheetId() === sheet.getSheetId(); })) {
      candidates.push(sheet);
    }
  });

  const rawPreferred = candidates.filter(function(sheet) {
    const name = String(sheet.getName() || "");
    return name.indexOf("Source - ") === 0 || /^(B\s|CD\s|UC\s|RD\s)/i.test(name);
  }).concat(candidates.filter(function(sheet) {
    const name = String(sheet.getName() || "");
    return name.indexOf("Source - ") !== 0 && !/^(B\s|CD\s|UC\s|RD\s)/i.test(name);
  }));

  for (let i = 0; i < rawPreferred.length; i++) {
    const sheet = rawPreferred[i];
    const name = String(sheet.getName() || "").trim();
    if (preferredOutputName && name === preferredOutputName) continue;
    try {
      validator(sheet);
      if (i > 0) {
        logBestEffortWarning_(workflowName + " selected monthly source sheet because the active sheet was not valid: " + name);
      }
      return sheet;
    } catch (err) {
      // Try the next same-month candidate before reporting the original active-sheet problem.
    }
  }

  throw new Error("No valid same-month source sheet was found for " + workflowName + ". Select the imported source tab for " + monthLabel + " and rerun.");
}
...
function applyStandardFormatting_(sheet, options) {
  ...
  try {
    ...
    logBestEffortWarning_("Cosmetic standard formatting skipped for " + sheet.getName() + ": " + err.message);
  }
  return sheet;
}
...
function functionSourceContainsAll_(fn, tokens) {
...
function buildCombinedFrameworkTestDashboardRows_() {
  return [
    ["TEST - Quality Report", "- v" + MASTER_LIST_MERGE_ML_VERSION + " -", "Report Formatter Framework"],
    [""],
...
function applyDashboardTemplateFormattingToActiveReportSheet_(targetSheet, dashboard, sheetDef, headers, behavior, monthParts, endDate, statusText, timing) {
  ...
  const colCount = Math.max(headers.length, 4);
  const rowCount = Math.max(targetSheet.getMaxRows(), resolveTemplateRowCount_(sheetDef, dashboard.globals));
...
function applyDashboardSortOrderAlternatingColors_(sheet) {
  sheet = sheet || SpreadsheetApp.getActiveSheet();
  if (!sheet) return;
  const lastRow = sheet.getLastRow();
  ...
  const headers = getHeaders_(sheet, HEADER_ROW);
  const primarySortIdx = headers.indexOf("Primary Row Sort Order");
  const sortIdx = headers.indexOf("Sort Order");
  if (primarySortIdx === -1 && sortIdx === -1) return;
  const sheetType = getSheetTypeForOrganization_(sheet.getName());
  const dashboard = loadDashboardConfig_();
  const globals = dashboard.globals || {};
  const sheetDef = getSheetDefinitionByType_(dashboard, sheetType);
  const theme = getThemeColorsFromBase_((sheetDef && sheetDef.baseColor) || "#7665CC", globals);
  const dataRowCount = lastRow - DATA_START_ROW + 1;
  const dataRange = sheet.getRange(DATA_START_ROW, 1, dataRowCount, lastCol);
  const primarySortValues = primarySortIdx !== -1
    ? sheet.getRange(DATA_START_ROW, primarySortIdx + 1, dataRowCount, 1).getValues()
    : null;
  const sortValues = sortIdx !== -1
    ? sheet.getRange(DATA_START_ROW, sortIdx + 1, dataRowCount, 1).getValues()
    : null;
  const backgrounds = [];
  for (let i = 0; i < dataRowCount; i++) {
    const rawPrimarySort = primarySortValues ? primarySortValues[i][0] : "";
    const rawSort = sortValues ? sortValues[i][0] : "";
    let group = Number(rawPrimarySort);
    if (isNaN(group) || rawPrimarySort === "") {
      const sortNumber = Number(rawSort);
      group = isNaN(sortNumber) ? i : Math.floor(sortNumber);
    }
    backgrounds.push(new Array(lastCol).fill(group % 2 === 0 ? theme.level4 : "#FFFFFF"));
  }
  try {
...
function assertSheetCanBeDeleted_(ss, sheet, context, additionalProtectedNames) {
  ...
  const sheetName = sheet.getName();
  if (isProtectedSheetDeletionName_(sheetName, additionalProtectedNames)) {
    throw new Error("Refusing to delete protected sheet" + (context ? " during " + context : "") + ": " + sheetName);
  }
  if (ss.getSheets().length <= 1) {
    throw new Error("Cannot delete the only sheet in the workbook" + (context ? " during " + context : "") + ": " + sheetName);
  }
}
...
function validateStagedMasterListBeforeSwap_(sheet, masterName, copiedRowCount) {
  ...
  if (!isStagedMasterListSheet_(sheet, masterName)) {
    throw new Error("Refusing to promote unexpected staged Master List sheet: " + sheet.getName());
  }
  const lastRow = sheet.getLastRow();
  const lastCol = sheet.getLastColumn();
...
function hideMonthlySheetsBySpecs_(processName, parsedMonth, specs) {
  return runFrameworkTimed_(processName + " " + parsedMonth.monthLabel + " & Prior", function(timing) {
    ...
    specs.forEach(function(spec) {
      // Target the prompted month AND all prior historical months in the workbook
...
function archiveMonthlyActiveSheets() {
  ...
  return runFrameworkTimed_("Archive Monthly Active Sheets " + parsed.monthLabel + " & Prior", function(timing) {
    ...
    refreshIndexAfterSheetWorkflow_("Archive Monthly Active Sheets");
...
function findArchiveMonthlyCandidateSheets_(ss, keywords, monthLabel) {
  ...
  ss.getSheets().forEach(function(sheet) {
    ...
    const lowerName = name.toLowerCase();
    if (lowerName.indexOf("template") !== -1) return;
    if (lowerName.indexOf(normalizedMonth) === -1) return;
    const hasKeyword = keywordList.some(function(keyword) { return lowerName.indexOf(keyword) !== -1; });
    if (!hasKeyword) return;
    if (seen.has(name)) return;
    seen.add(name);
...
function copySheetToArchiveAndDeleteLocal_(ss, archiveSs, sourceSheet, archiveName, timing) {
  ...
  if (isProtectedSheetDeletionName_(sourceSheetName)) {
    hideSheetIfNeeded_(sourceSheet, timing, "Archived protected local sheet hidden instead of deleted: " + sourceSheetName);
...
function deleteArchiveSheetIfExists_(archiveSs, sheetName) {
  ...
  if (isProtectedSheetDeletionName_(sheetName)) {
    throw new Error("Refusing to replace protected archive sheet: " + sheetName);
  }
...
function formatRawData() {
  ...
  assertActiveRawDataSourceSheet_(sourceSheet, "Format Raw Data");
  const monthParts = promptForLockedYearReportMonth_("Format Raw Data");
  if (!monthParts) return;

  return runFrameworkTimed_("Format Raw Data " + formatReportDateLabel_(monthParts.firstDay), function(timing) {
    const outputSheet = formatRawDataInPlaceSheet_(sourceSheet, monthParts, timing, markFrameworkStep_);

    notify_(
      "Raw Data formatted in place as " + outputSheet.getName() + ".\n" +
      "Imported Raw Data columns and values were retained on the source sheet.\n" +
      "Four title/header governance rows and approved framework columns were added when missing.\n" +
      "Primary PMR Row and Banner fields were added/populated. Run Process Demo P next when ready."
    );
...
function formatRawDataInPlaceSheet_(sourceSheet, monthParts, timing, markStepFn, options) {
  ...
  if (sourceName === sheetDef.templateName) {
  ...
  const context = buildRawDataInPlaceFormattingContext_(sourceSheet, dashboard, sheetDef, monthParts);
  step(timing, "Read Raw Data source in place without editing imported values | Rows: " + context.rawData.values.length);

  deleteSheetIfExists_(ss, outputName, sourceName, sheetDef.templateName);
  if (sourceSheet.getName() !== outputName) {
    sourceSheet.setName(outputName);
    placeCreatedSheetInConfiguredOrder_(sourceSheet);
    step(timing, "Rename Raw Data import sheet in place and place in configured creation order: " + sourceName + " -> " + outputName);
  }

  const inPlaceResult = writeRawDataInPlaceTitleRowsAndAddedColumns_(sourceSheet, context, monthParts, timing, step);
  step(timing, "Wave 4 Raw Data in-place title/header/column work complete | Added columns: " + inPlaceResult.addedColumns.join(", "));

  step(timing, "Wave 4 Raw Data approved-sync columns start");
  const approvedColumnsWritten = processRawDataApprovedSyncColumns_(sourceSheet, monthParts, timing, step);
  step(timing, "Wave 4 Raw Data approved-sync columns complete | Changed column groups: " + approvedColumnsWritten);
...
function writeRawDataInPlaceTitleRowsAndAddedColumns_(sheet, context, monthParts, timing, markStepFn) {
  ...
  step(timing, "Wave 4 Raw Data header row detection complete | Header row: " + originalHeaderRow);
  ...
  if (originalHeaderRow < HEADER_ROW) {
    ...
    step(timing, "Wave 4 Raw Data title-row insert complete | Rows inserted: " + (HEADER_ROW - originalHeaderRow));
  } else {
    step(timing, "Wave 4 Raw Data title-row insert skipped");
  }


  const headerWidth = Math.max(sheet.getLastColumn(), 1);
  const existingHeaders = sheet.getRange(HEADER_ROW, 1, 1, headerWidth).getValues()[0]
    .map(function(header) { return String(header || "").trim(); });
  ...
  if (addedColumns.length) {
    ...
    step(timing, "Wave 4 Raw Data approved-column append complete | Columns added: " + addedColumns.length);
  } else {
    step(timing, "Wave 4 Raw Data approved-column append skipped");
  }


  const finalWidth = Math.max(sheet.getLastColumn(), headerWidth + addedColumns.length, 1);
  const finalHeaders = sheet.getRange(HEADER_ROW, 1, 1, finalWidth).getValues()[0]
    .map(function(header) { return String(header || "").trim(); });


  step(timing, "Wave 4 Raw Data title/header formatting start | Width: " + finalWidth);
  ...
  step(timing, "Wave 4 Raw Data title/header formatting complete");


  try {
    sheet.setTabColor(sheetDef.baseColor || sheet.getTabColor());
  } catch (err) {
    logBestEffortWarning_("In-place Raw Data tab color skipped for " + sheet.getName() + ": " + err.message);
  }


  try {
    sheet.setFrozenRows(HEADER_ROW);
    step(timing, "Wave 4 Raw Data frozen rows set");
    const existingFilter = sheet.getFilter && sheet.getFilter();
    if (existingFilter) {
      existingFilter.remove();
      step(timing, "Wave 4 Raw Data filter removal complete");
    } else {
      step(timing, "Wave 4 Raw Data filter removal skipped - no existing filter");
    }
    const filterRows = Math.max(sheet.getMaxRows() - HEADER_ROW + 1, 1);
    sheet.getRange(HEADER_ROW, 1, filterRows, finalWidth).createFilter();
    step(timing, "Wave 4 Raw Data filter creation complete | Rows: " + filterRows + "; Columns: " + finalWidth);
  } catch (err) {
    logBestEffortWarning_("In-place Raw Data filter/freeze setup skipped for " + sheet.getName() + ": " + err.message);
  }
  ...
  return {
    ...
    governedRawDataHeaders: governedRawDataHeaders,
...
function applyUniversalFastCanvasFormatting_(sheet, dashboard, headers) {
  ...
  try {
    ...
    throw new Error("Universal canvas formatting failed for " + sheet.getName() + ": " + err.message);
  }
}
...
function applyGovernedTextAndNumberFormats_(sheet, dashboard, headers, dataStartRow, rowCount) {
  ...
  const hasExplicitRowCount = rowCount !== undefined && rowCount !== null;
  const requestedRows = hasExplicitRowCount ? Number(rowCount || 0) : Number(sheet.getMaxRows() - startRow + 1);
  ...
  headers.forEach(function(header, index) {
    ...
    if (!format) {
      format = columnDef.dateColumn || isDateLikeHeader_(header) ? (globals.defaultDateFormat || "mm/dd/yyyy") : "@";
    }
    const column = index + 1;
    const rangeA1 = rowColToA1_(startRow, column) + ":" + rowColToA1_(startRow + rows - 1, column);
  ...
  Object.keys(formatsByA1).forEach(function(format) {
    try {
      sheet.getRangeList(formatsByA1[format]).setNumberFormat(format);
    } catch (err) {
      throw new Error("Governed text/number format failed for " + sheet.getName() + " with format " + format + ": " + err.message);
    }
  });
}
...
function testCreateOutputSheetByCopying_(sheetType, outputName, outputRows, firstDay, endDate, timing, timingLabel) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const normalizedSheetType = normalizeDashboardSheetTypeKey_(sheetType);
  const context = getTemplateDrivenActiveSheetContext_(ss, normalizedSheetType);
  const dashboard = context.dashboard || loadDashboardConfig_();
  const sheetDef = context.sheetDef;
  const template = context.template;
  if (!template) {
    throw new Error("TEST template not found: " + sheetDef.templateName + ". Run Build Test Templates first.");
  }

  const templateHeaders = getHeaders_(template, HEADER_ROW);
  if (!templateHeaders.some(function(header) { return String(header || "").trim(); })) {
    throw new Error("TEST template is missing its Section H headers on row " + HEADER_ROW + ": " + sheetDef.templateName);
  }

  const safeOutputName = ensureTestSheetPrefix_(outputName);
  deleteSheetIfExists_(ss, safeOutputName, template.getName());
  
  // DUPLICATE TEMPLATE (Inherits Native Banding Automatically)
  const sheet = template.copyTo(ss);
  sheet.setName(safeOutputName);
  sheet.showSheet();
  ss.setActiveSheet(sheet);

  const width = Math.max(templateHeaders.length, 1);
  const rows = (outputRows || []).map(function(row) {
    const output = (row || []).slice(0, width);
    while (output.length < width) output.push("");
    return output;
  });
  const requiredRows = Math.max(DATA_START_ROW + Math.max(rows.length, 1) - 1, DATA_START_ROW);
  resizeSheetGrid_(sheet, requiredRows, width);
  
  // Write headers and data values without altering merges or bandings
  sheet.getRange(HEADER_ROW, 1, 1, width).setValues([templateHeaders]);
  if (rows.length) sheet.getRange(DATA_START_ROW, 1, rows.length, width).setValues(rows);

  sheet.getRange("A1").setValue(sheetDef.reportTitle || normalizedSheetType || safeOutputName);
  if (firstDay) sheet.getRange("B2").setValue(firstDay);
  if (endDate) sheet.getRange("D2").setValue(endDate);

  // NOTE: Native banding re-application removed here because it is inherited directly from template.copyTo()

  const existingFilter = sheet.getFilter && sheet.getFilter();
  if (existingFilter) existingFilter.remove();
  sheet.getRange(HEADER_ROW, 1, Math.max(sheet.getMaxRows() - HEADER_ROW + 1, 1), width).createFilter();
  sheet.getRange(DATA_START_ROW, 1, Math.max(rows.length, 1), width)
    .setWrapStrategy(SpreadsheetApp.WrapStrategy.CLIP);
    
  clearSheetRuntimeCachesForSheet_(sheet);
  if (timing) markFrameworkStep_(timing, String(timingLabel || "TEST template copy") + ": " + safeOutputName);
  return sheet;
}


function ensureTestSheetPrefix_(sheetName) {
  const name = String(sheetName || "Report").trim();
  return safeSheetName_(name.indexOf("TEST ") === 0 ? name : "TEST " + name);
}

function createOutputSheetFromDashboardTemplate_(sheetType, outputName, outputRows, firstDay, endDate, timing, timingLabel) {
  return testCreateOutputSheetByCopying_(sheetType, outputName, outputRows, firstDay, endDate, timing, timingLabel);
}

function createRawDataOutputSheetFromTemplateFast_() {
  throw new Error("TEST Raw Data must be formatted in place; template duplication is disabled.");
}
...
function syncRawDataBannerColumns_(rawSheet, monthParts, timing, markStepFn) {
  if (!rawSheet) return 0;
  const step = typeof markStepFn === "function" ? markStepFn : markRuntimeStep_;
  const bannerSheet = getCurrentBannersSheet_(monthParts);
  if (!bannerSheet) {
    logBestEffortWarning_("Raw Data Banner sync skipped: Banners sheet not found.");
    return 0;
  }


  const data = getDataValues_(rawSheet, HEADER_ROW, DATA_START_ROW);
  step(timing, "Wave 4 Raw Data banner-sync bulk read complete | Rows: " + data.values.length);
  if (!data.values.length) return 0;


  const rawHeaderMap = data.headerMap;
  const rawPMRIdx = getPMRIndex_(rawHeaderMap);
  const rawPrimaryPMRRowIdx = rawHeaderMap["Primary PMR Row"];
  const rawLastNameIdx = rawHeaderMap["Last Name"];
  const rawFirstNameIdx = rawHeaderMap["First Name"];
  if (rawPMRIdx === -1 || rawLastNameIdx === undefined || rawFirstNameIdx === undefined) {
    logBestEffortWarning_("Raw Data Banner sync skipped: missing Participant PMR#, Last Name, or First Name.");
    return 0;
  }
  ...
  step(timing, "Wave 4 Raw Data banner-sync source map built | Keys: " + bannerMap.size);
  if (bannerMap.size === 0) return 0;


  const beforeValues = {};
  BANNER_SYNC_FIELDS.forEach(function(header) {
    const idx = rawHeaderMap[header];
    if (idx !== undefined) {
  ...
  data.values.forEach(function(row) {
    if (rawPrimaryPMRRowIdx !== undefined && !isPrimaryPMRRowValue_(row[rawPrimaryPMRRowIdx])) return;
    ...
    BANNER_SYNC_FIELDS.forEach(function(field) {
      const targetIdx = rawHeaderMap[field];
      if (targetIdx === undefined) return;
      if (!Object.prototype.hasOwnProperty.call(source, field)) return;
      const newValue = source[field];
      const cleanOld = String(row[targetIdx] || "").trim().toUpperCase();
      const cleanNew = String(newValue || "").trim().toUpperCase();
      if (cleanOld !== cleanNew) {
        row[targetIdx] = newValue;
        changed = true;
      }
    });
  ...
  if (updatedRows > 0) {
    const changedColumnGroups = writeChangedColumnsOnly_(rawSheet, data, beforeValues);
    step(timing, "Wave 4 Raw Data banner-sync guarded write complete | Changed column groups: " + changedColumnGroups);
    clearSheetRuntimeCachesForSheet_(rawSheet);
  } else {
    step(timing, "Wave 4 Raw Data banner-sync guarded write skipped - no changed rows");
  }
  return updatedRows;
}
...
function writeChangedColumnsOnly_(sheet, data, beforeValues) {
  ...
  // Demo P and Raw Data guarded updates must never widen into nearby unchanged columns.
  // Only truly contiguous changed indexes are written together, preserving intact data.
  const groups = [];
...
function isOngoingOutputSheetType_(sheetType) {
  ...
  return value === SHEET_TYPE.DEMO_P ||
    value === RFF_SHEET_TYPES.DEMO_P ||
    value === SHEET_TYPE.DISENROLLED_EXCLUSION ||
    value === RFF_SHEET_TYPES.DISENROLLED_EXCLUSION;
}
...
function buildDashboardOutputSheetName_(sheetDef, monthParts) {
  ...
  return ensureTestSheetPrefix_(safeSheetName_(name));
}
...
function syncRowsFromSourceMapData_(data, sourceMap, config, pmrFilter) {
  ...
  data.values.forEach(function(row) {
    ...
    const key = config.masterKeyHeaders
      .map(function(header, i) { return normalizeSyncKey_(row[headerMap[header]], targetKeyHeaders[i]); })
      .join("|||");
...
function buildSourceMapByCompositeKeyForPart5_(sheet, headerRow, dataStartRow, keyHeaders) {
  ...
  data.values.forEach(function(row) {
    const key = keyHeaders
      .map(function(header) { return normalizeSyncKey_(row[headerMap[header]], header); })
      .join("|||");
...
function isStrictRawDataSheetCandidateForDemoP_(sheet, monthParts) {
  ...
  if (sheetName === "TEST Template - Raw Data") return false;
...
function updateDemoPMonthlySync() {
  ...
  return runFrameworkTimed_("Update Demo P (Monthly Change Sync) " + formatReportDateLabel_(monthParts.firstDay), function(timing) {
    const demoSheet = updateDemoPMonthlySyncForMonth_(monthParts, timing, { timingPrefix: "" });
    markFrameworkStep_(timing, "Update Demo P monthly sync workflow complete: " + (demoSheet ? demoSheet.getName() : "No Demo P update"));
    notify_("Update Demo P (Monthly Change Sync) complete." + (demoSheet ? "\n\nDemo P: " + demoSheet.getName() : ""));
    return demoSheet;
  });
}
...
function validateDemoPMonthlySyncReplacementCoverage_(changedPMRs, freshRows, headers) {
  ...
  const missingPMRs = [];
  changedPMRs.forEach(function(pmr) {
    if (!replacementPMRs.has(pmr)) missingPMRs.push(pmr);
  });
...
function normalizeRowsToWidth_(rows, width) {
  return (rows || []).map(function(row) {
    return normalizeRowToWidth_(row, width);
  });
}
...
function updateDemoPReportDates_(demoSheet, monthParts) {
  ...
  demoSheet.getRange("E2").setValue("Last Updated")
    .setFontFamily("Arial")
    .setFontSize(10)
    .setFontStyle("italic")
    .setHorizontalAlignment("left");
...
function enforceDemoPPostFlattenFormatting_(demoSheet) {
  ...
  // High-speed strict alphabetical participant sort core
...
function sortSheetAlphabeticallyByParticipantName_(sheet) {
  ...
  // Execute bulk UI sort pass via Sheets API on data boundaries
...
function getDemoPMonthlySyncChangedPMRs_(monthParts, timing) {
  ...
  const pmrIdx = getPMRIndex_(data.headerMap);
  if (pmrIdx === -1) {
    throw new Error("Monthly Change Report is missing Participant PMR# / PMR # column.");
  }
...
function buildDemoPFreshRowsForPMRs_(rawSheet, demoHeaders, changedPMRs, monthParts, timing) {
  ...
  const flatCount = flattenDemoPContactRowsInMemory_(workingData, false);
  markRuntimeStep_(timing, "Changed Demo P rows processed and flattened from Raw Data in memory | Rows retained: " + flatCount);
...
function flattenDemoPContactRowsInMemory_(data, requireIntegrity) {
  ...
  const pmrIdx = getPMRIndex_(headerMap);
  const primaryIdx = headerMap["Primary PMR Row"];
  if (pmrIdx === -1 || primaryIdx === undefined) throw new Error("Demo P in-memory flat-record compression requires PMR and Primary PMR Row headers.");
  ...
  data.values.forEach(function(row, originalIndex) {
    ...
    grouped.get(pmr).push({ row: row, originalIndex: originalIndex });
  });
  ...
  grouped.forEach(function(items, pmr) {
    ...
    const output = primaryItem.row.slice();
    const summaries = [];
...
function processDemoP() {
  ...
  try {
    ...
    placeCreatedSheetInConfiguredOrder_(sheet);
    markRuntimeStep_(timing, "Maintain ongoing Demo P sheet name and place behind Index");
...
function processDemoPAsWorkingSource_(sheet, monthParts, timing, sourceSheetName, status) {
  ...
  applyAlternatingColorsForSheetType_(sheet, loadDashboardConfig_(), SHEET_TYPE.DEMO_P, DATA_START_ROW + data.values.length - 1, data.headers.length, null, "");
  markRuntimeStep_(timing, "Demo P Working Source - alternating colors reapplied");
...
function updateExistingDemoPFromRawData_(demoSheet, rawSheet, monthParts, timing) {
  ...
  if (workingData.values.length) {
    ...
    applyAlternatingColorsForSheetType_(demoSheet, dashboard, SHEET_TYPE.DEMO_P, DATA_START_ROW + workingData.values.length - 1, headers.length, timing, "Demo P existing update - alternating colors applied");
  }
...
function createActiveDemoPFromRawData_(rawSheet, targetName, monthParts, timing) {
  ...
  const flatCount = flattenDemoPContactRowsInMemory_(workingData, true);
  ...
  const demoSheet = testCreateOutputSheetByCopying_(
    SHEET_TYPE.DEMO_P,
    targetName,
    workingData.values,
    monthParts && monthParts.firstDay,
    monthParts && monthParts.lastDay,
    timing,
    "Demo P TEST template copy"
  );
  demoSheet.getRange("A1").setValue(sheetDef.reportTitle || "Demo P");
...
function buildMonthlyChangeReport() {
  ...
  return runFrameworkTimed_("Build Monthly Change Report " + formatReportDateLabel_(monthParts.firstDay), function(timing) {
    ...
    markFrameworkStep_(timing, reportSheet ? "Monthly Change workflow complete" : "Monthly Change workflow stopped without report");
    return reportSheet;
  });
}
...
function compareRawDemoPForSectionReport_(previousDemo, currentDemo, monthParts) {
  ...
  currentData.rowsByPMR.forEach((currentRows, pmr) => {
    ...
    const currentPMRSignature = buildRowsSignatureForCompare_(currentRows, currentData.headerMap, allCompareFields);
    const previousPMRSignature = buildRowsSignatureForCompare_(previousRows, previousData.headerMap, allCompareFields);
...
function buildRowsSignatureForCompare_(items, headerMap, columnsToCompare) {
  const columnIndexes = (columnsToCompare || [])
    .map(function(header) { return headerMap[header]; })
    .filter(function(idx) { return idx !== undefined && idx !== -1; });
  if (!items || items.length === 0 || columnIndexes.length === 0) return "";
  return items
    .map(function(item) {
      const values = item.values || [];
      return columnIndexes.map(function(idx) {
        return normalizeCompareValue_(values[idx]);
      }).join("|~|");
    })
    .sort()
    .join("|~~|");
...
function buildMonthlyChangeReportSectionLayout_(reportSheet, sourceSheet, headers, monthParts) {
  ...
  // Preserve governed top-of-report metadata from the copied Monthly Change template.
...
function updateDemoPMonthlySyncForMonth_(monthParts, timing, options) {
  ...
  // PREFLIGHT GUARD: Soft check before starting execution
  ...
  if (!mcrSheet) {
    ...
    if (response === ui.Button.YES) {
      ...
      if (!mcrSheet) {
        ...
        return null; // Graceful return prevents CRITICAL timing flag
      }
    } else {
...
function createDisenrolledListForMonth_(monthParts, timing, options) {
  ...
  const result = moveDisenrolledPMRsFromDemoPToExclusion_(demoSheet, monthParts, timing);
  step("Copy disenrolled primary records and remove rows: " + result.rowsCopied + " copied, " + result.rowsRemoved + " removed");
  setLastRawDataDisenrolledBuildResult_(result);

  const exclusionSheet = ss.getSheetByName(DISENROLLED_EXCLUSION_SHEET);
  const oldRowsHidden = hideOldDisenrolledRows_(exclusionSheet);
  step("old rows hidden", "Rows: " + oldRowsHidden);

  // WAVE 4 OPTIMIZATION: Only refresh index if explicitly requested for standalone executions
  if (options.refreshIndex === true) {
    refreshIndexAfterSheetWorkflow_(options.workflowName || "Create Disenrolled List");
    step("Index refreshed after Create Disenrolled List");
  } else {
    step("Index refresh deferred until final organization");
  }

  if (options.notify) {
    notify_("Disenrolled Exclusion List structural refresh executed successfully.");
    step("User notification sent after Create Disenrolled List");
  }
  return result;
}
...
function createMasterList() {
  ...
  return runFrameworkTimed_("Create Master List", function(timing) {
    return createMasterListForMonth_(monthParts, timing);
  });
}
...
function createMasterListForMonth_(monthParts, parentTiming, preflight) {
  ...
  const markStep = function(label, details) {
    if (parentTiming) {
      markFrameworkStep_(timing, "Create Monthly Update - Create Master List - " + label, details || "");
    } else {
      markRuntimeStep_(timing, label, details);
    }
  };
  ...
  try {
    ...
    if (existingMasterSheet) {
      ...
      markStep("Copy native-banded TEST Master List template");
    }

    buildMasterListHeadersBeforeDataCopy_(demoSheet, masterSheet);
    markStep("Build Master List headers");

    // WAVE 4 OPTIMIZATION: Compile data and sync Care Plans in memory BEFORE writing
    ...
    markStep("Mapped Primary PMR rows in memory | Rows: " + copiedRowCount);

    if (copiedRowCount === 0) {
      throw new Error("Cannot create Master List: No Primary PMR rows found. Primary PMR Row assignment is handled during Raw Data formatting.");
    }

    const inMemoryData = {
      headers: masterHeaders,
      headerMap: masterHeaderMap,
      values: mappedRows,
      range: null
    };
    ...
    // Single unified batch write to sheet
    trimOutputSheetToDataSize_(masterSheet, copiedRowCount, SHEET_TYPE.MASTER_LIST);
...
// Helper function to extract rows into memory without writing to sheet first
function buildPrimaryDemoPRowsInMemory_(demoSheet, masterHeaders, masterHeaderMap) {
  ...
  demoData.values.forEach(row => {
    const pmr = normalizePMR_(row[pmrIdx]);
    if (!pmr) return;
    const includeRow = primaryIdx !== undefined && isPrimaryPMRRowValue_(row[primaryIdx]);
...
function copyPrimaryDemoPRowsToMasterListByHeader_(demoSheet, masterSheet, monthParts) {
  ...
  const output = buildOutputRows_();

  if (output.length === 0) {
    throw new Error("Cannot create Master List: No Primary PMR rows found. Primary PMR Row assignment is handled during Raw Data formatting. Please review and reformat the Raw Data sheet for this month before proceeding.");
  }
...
function createMasterListSheetFromTemplate_(ss, targetName, monthParts, timing, timingLabel) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  const sheet = testCreateOutputSheetByCopying_(
    ...
    monthParts && monthParts.lastDay,
    timing,
    timingLabel || "TEST Master List template copy"
  );
...
/**
 * Checks if the Index sheet already has a valid header/title shell structure.
 */
...
/**
 * Stage 1: Builds the static Index header, merges, frozen rows, and column widths ONCE.
 */
...
/**
 * Fast In-Place Update: Local Operational Workspace (Columns A-D).
 * Buckets active tabs under a single Header per category in specified hierarchy.
 */
...
/**
 * In-Place Update: Cold-Storage Archive Workspace (Columns F-J).
 * Maintains monthly grouping. Runs during archive/restore operations.
 */
...
/**
 * Main Index Generator & In-Place Data Update Router.
 * Options:
 *  - activeOnly: true        -> Refreshes Columns A-D only (Fast path)
 *  - archiveOnly: true       -> Refreshes Columns F-J only
 *  - forceShellRebuild: true -> Forces full shell/header teardown and rebuild
 *  - archiveSs: sheetObject  -> Reuses open connection
 */
 ...
 * back into the active Master List workbook based on row selections or a routed target name.
 */
...
// Internal implementation intentionally uses a distinct name so the
// hideSystemSheets_ menu callback remains a single stable wrapper.
...
function createDisenrolledList() {
  ...
  return runFrameworkTimed_("Create Disenrolled List", function(timing) {
    return createDisenrolledListForMonth_(monthParts, timing, {
      ...
      refreshIndex: true,
      notify: true,
      workflowName: "Create Disenrolled List"
    });
  });
}
...
function moveDisenrolledPMRsFromDemoPToExclusion_(demoSheet, monthParts, timing) {
  const result = { pmrsRemoved: 0, rowsRemoved: 0, rowsCopied: 0 };
  const step = function(label) { if (timing) markFrameworkStep_(timing, "Disenrolled move - " + label); };
  ...
  step("exclusion sheet ready: " + exclusionSheet.getName());
  ...
  step("existing exclusion data read | Rows: " + existingExclusionData.values.length);
  ...
  step("Demo P body bulk read | Rows: " + values.length);
  ...
  step("Demo P disenrollment scan complete | Disenrolled PMRs: " + workspaceDisenrolledPMRs.size + "; active re-enrolled PMRs: " + activeReEnrolledPMRs.size);
  ...
  if (activeReEnrolledPMRs.size > 0 && exclusionPmrIdx !== -1) {
    ...
    if (exclusionRowsToDelete.length > 0) {
      ...
      step("re-enrolled exclusion rows purged | Rows: " + exclusionRowsToDelete.length);
  ...
  values.forEach(function(row) {
    const pmr = normalizePMR_(row[pmrIdx]);
    if (!pmr || !workspaceDisenrolledPMRs.has(pmr)) {
      retainedRows.push(normalizeRowToWidth_(row, lastCol));
      return;
    }
  ...
  step("Demo P retained/remove partition complete | Retained rows: " + retainedRows.length + "; rows selected: " + rowsRemoved);
  result.pmrsRemoved = workspaceDisenrolledPMRs.size;
  result.rowsRemoved = rowsRemoved;

  result.rowsCopied = appendDisenrolledDeltasToExclusionSheet_(exclusionSheet, headers, rowsToAppendByPMR, monthParts, timing);
  if (result.rowsCopied !== result.rowsRemoved) {
    throw new Error("Disenrollment stopped before changing Demo P. Rows copied to Disenrolled Exclusion (" + result.rowsCopied + ") did not match rows selected for removal (" + result.rowsRemoved + ").");
  }

  writeDemoPDisenrollmentRetainedBody_(demoSheet, retainedRows, lastCol, timing);

  const remainingActiveRows = Math.max(DATA_START_ROW + retainedRows.length - 1, DATA_START_ROW + 50);
  forceSheetRowCount_(demoSheet, Math.max(remainingActiveRows, 500));
  step("Demo P row count normalized after retained rewrite");
...
function writeDemoPDisenrollmentRetainedBody_(demoSheet, retainedRows, width, timing) {
  const step = function(label) { if (timing) markFrameworkStep_(timing, "Disenrolled move - Demo P retained rewrite - " + label); };
  const normalizedRows = normalizeRowsToWidth_(retainedRows || [], width);
  step("rows normalized | Rows: " + normalizedRows.length);
  const oldBodyRows = Math.max(demoSheet.getLastRow() - DATA_START_ROW + 1, 0);
  const rowsToClear = Math.max(oldBodyRows, normalizedRows.length, 1);
  const colsToClear = Math.max(demoSheet.getLastColumn(), width, 1);
...
function updateDisenrolledExclusionReportDates_(sheet, monthParts) {
  if (!sheet) return;
  ...
  sheet.getRange("E2").setValue("Last Updated")
    .setFontFamily("Arial")
    .setFontSize(10)
    .setFontStyle("italic")
    .setHorizontalAlignment("left");
...
function appendDisenrolledDeltasToExclusionSheet_(exclusionSheet, sourceHeaders, rowsToAppendByPMR, monthParts, timing) {
  ...
  try {
    ...
    if (formatSourceRow <= exclusionSheet.getMaxRows()) {
      ...
      applyAlternatingColorsForSheetType_(exclusionSheet, loadDashboardConfig_(), SHEET_TYPE.DISENROLLED_EXCLUSION, exclusionSheet.getLastRow(), width, null, "");
      step("Disenrolled Exclusion alternating colors reapplied");
...
function writeDisenrolledAppendPayloadValues_(sheet, startRow, payload, width) {
  const rowCount = payload ? payload.length : 0;
  const result = {
    columnsWritten: 0,
    runsWritten: 0,
    blankColumnsSkipped: 0
  };
  if (!sheet || rowCount === 0 || width < 1) return result;

  const nonBlankColumns = [];
  for (let col = 0; col < width; col++) {
    let hasValue = false;
    for (let row = 0; row < rowCount; row++) {
      if (normalizeCompareValue_(payload[row][col]) !== "") {
        hasValue = true;
        break;
      }
    }
    if (hasValue) {
      ...
      result.blankColumnsSkipped++;
    }
  }

  if (nonBlankColumns.length === 0) return result;

  let runStart = nonBlankColumns[0];
  let previous = runStart;
  const flushRun = function(startCol, endCol) {
    const runWidth = endCol - startCol + 1;
    const values = payload.map(function(row) {
      return row.slice(startCol, endCol + 1);
    });
    sheet.getRange(startRow, startCol + 1, rowCount, runWidth).setValues(values);
    result.columnsWritten += runWidth;
    result.runsWritten++;
  };

  for (let i = 1; i < nonBlankColumns.length; i++) {
    const col = nonBlankColumns[i];
    if (col === previous + 1) {
      previous = col;
      continue;
    }
...
function appendDisenrolledPMRBlocksToExclusion_(sourceSheet, sourceHeaders, rowsByPMR, monthParts) {
  ...
  return appendDisenrolledDeltasToExclusionSheet_(exclusionSheet, sourceHeaders, rowsByPMR, monthParts, null);
}
...
function createDisenrolledExclusionSheetFromDashboardTemplate_(ss, monthParts) {
  ...
  const dashboard = context.dashboard || loadDashboardConfig_();
  const template = context.template;
  let sheet = template.copyTo(ss);
  setUniqueSheetName_(sheet, DISENROLLED_EXCLUSION_SHEET);
  placeCreatedSheetInConfiguredOrder_(sheet);
  showSheetIfNeeded_(sheet, null, null);

  updateDisenrolledExclusionReportDates_(sheet, monthParts);
  applyAlternatingColorsForSheetType_(sheet, dashboard, SHEET_TYPE.DISENROLLED_EXCLUSION, sheet.getMaxRows(), Math.max(sheet.getLastColumn(), 1), null, "");
  clearSheetRuntimeCachesForSheet_(sheet);
  return sheet;
}
...
function getCurrentDemoPSheet_(monthParts) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const workingSheet = ss.getSheetByName(DEMO_P_PREFIX);
  if (workingSheet) return workingSheet;
...
function getMonthlyChangeReportDateIndexes_(reportHeaders) {
  return GLOBAL_DATE_FORMAT_HEADERS
    .map(function(headerName) { return reportHeaders.indexOf(headerName); })
...
function formatMonthlyChangeReportSectionSheet_(reportSheet, sourceHeaders) {
  ...
  // Strip native alternating-color bandings before any custom in-memory paint arrays are used.
  stripMonthlyChangeNativeBandings_(reportSheet);

  applyColumnWidths_(reportSheet, dashboard, reportHeaders);
  applyHiddenColumnSettings_(reportSheet, dashboard, reportHeaders);
  reportSheet.setFrozenRows(4);
  reportSheet.setFrozenColumns(2);
}
...
function collectFormatDashboardSheetDefinitionVerificationRows_(dashboardSheet) {
  ...
  sectionRows.forEach(function(row, index) {
    ...
    if (String(row[2] || "").indexOf("TEST Template - ") !== 0) rows.push([label, "FAIL", "Template Name Misaligned", "Column 3 must contain a Template - ... name."]);
...
function computeDashboardQualityShellSignature_(dashboard) {
  const parts = [
    "v=" + MASTER_LIST_MERGE_ML_VERSION,
    "sections=" + RFF_DASHBOARD_QUALITY_SECTIONS.map(function(section) {
      return section.key + ":" + section.title;
    }).join("||"),
    "width=" + RFF_DASHBOARD_QUALITY_COL_WIDTHS.join(",")
  ];
  if (dashboard && dashboard.sheetDefinitions) {
    parts.push("sheets=" + dashboard.sheetDefinitions.map(function(def) {
      return [def.sheetType, def.templateName, def.templateRowCount, def.templateColumnCount].join(":");
    }).join("||"));
  }
  return computeStableHash_(parts.join("|"));
}

function storeDashboardQualityShellSignature_(sheet, signature) {
  if (!sheet) return;
  try {
    sheet.getRange(1, 1).setNote("Format Signature: " + (signature || ""));
  } catch (err) {
    logBestEffortWarning_("Dashboard Quality shell signature note write skipped: " + err.message);
  }
}
...
function rebuildDashboardQualitySheetShellStructure_(sheet, dashboard, timing) {
  ...
  storeDashboardQualityShellSignature_(sheet, computeDashboardQualityShellSignature_(dashboard || null));
  markFrameworkStep_(timing, "Dashboard Quality matrix shell rebuilt and signature stamped");
}

function ensureDashboardQualitySheetShellForWorkflow_(sheet, dashboard, timing) {
  if (!sheet) return sheet;
  const storedSignature = getStoredTemplateFormatSignatureFromSheet_(sheet);
  const currentSignature = computeDashboardQualityShellSignature_(dashboard || null);
  const shellMatches = storedSignature !== "" && storedSignature === currentSignature && hasDashboardQualityTemplateShell_(sheet);
  if (!shellMatches) {
    rebuildDashboardQualitySheetShellStructure_(sheet, dashboard, timing);
  } else {
    markFrameworkStep_(timing, "Dashboard Quality matrix shell structure matches validation signature; layout clear bypassed");
...
function rebuildDashboardQualityReportShellCompact_(sheet) {
  ...
  const rows = [normalizeSectionRowForWidth_(["TEST - Quality Report", "- v" + MASTER_LIST_MERGE_ML_VERSION + " -", "Report Formatter Framework"], requiredCols)];
...
function hasDashboardQualityTemplateShell_(sheet) {
  ...
  if (titleValues[0] !== "TEST - Quality Report") return false;
  if (titleValues[1] !== "- v" + MASTER_LIST_MERGE_ML_VERSION + " -") return false;
  ...
  for (let i = 0; i < RFF_DASHBOARD_QUALITY_SECTIONS.length; i++) {
    if (!existingTitles.has(RFF_DASHBOARD_QUALITY_SECTIONS[i].title)) return false;
  }
...
function initializeDashboardQualitySheet_() {
  ...
  if (!sheet) {
    sheet = insertGovernedOutputSheet_(ss, RFF_TEST_DASHBOARD_SHEET);
  } else {
    showSheetIfNeeded_(sheet);
  }
...
function initializeSystemSheets_() {
  ...
  // Build system sheets in configured tail-block order.
  let dashboardSheet = ss.getSheetByName(RFF_DASHBOARD_SHEET);
  if (!dashboardSheet) {
    dashboardSheet = insertGovernedOutputSheet_(ss, RFF_DASHBOARD_SHEET);
  } else {
    placeCreatedSheetInConfiguredOrder_(dashboardSheet);
  }

  let qualitySheet = ss.getSheetByName(RFF_TEST_DASHBOARD_SHEET);
  if (!qualitySheet) {
    qualitySheet = insertGovernedOutputSheet_(ss, RFF_TEST_DASHBOARD_SHEET);
  } else {
    placeCreatedSheetInConfiguredOrder_(qualitySheet);
  }

  let timingSheet = ss.getSheetByName(RFF_TIMING_SHEET);
  if (!timingSheet) {
    timingSheet = insertGovernedOutputSheet_(ss, RFF_TIMING_SHEET);
  } else {
    placeCreatedSheetInConfiguredOrder_(timingSheet);
  }

  showSheetIfNeeded_(dashboardSheet);
  showSheetIfNeeded_(qualitySheet);
  showSheetIfNeeded_(timingSheet);

  if (!hasDashboardQualityTemplateShell_(qualitySheet)) {
    rebuildDashboardQualityReportShellCompact_(qualitySheet);
  } else {
    applyDashboardQualityReportColumnSettings_(qualitySheet, { skipRangeFormat: true });
  }

  if (!hasFrameworkTimingReportShell_(timingSheet)) {
    rebuildFrameworkTimingReportShellCompact_(timingSheet);
  } else {
    styleFrameworkTimingReport_(timingSheet);
  }

  let dashboardConfig = null;
  try {
    dashboardConfig = loadDashboardConfig_();
  } catch (err) {
    logBestEffortWarning_("Golden Master dashboard defaults fallback used: " + err.message);
  }

  // 4. Build Golden Master Base Template last.
  const baseTemplate = ensureGoldenMasterTemplate_(dashboardConfig, null);

  return {
    dashboardConfigSheet: dashboardSheet,
    frameworkTiming: timingSheet,
    dashboardQuality: qualitySheet,
...
function applyDashboardQualityReportColumnSettings_(sheet, options) {
  ...
  trimSheetToColumnCount_(sheet, RFF_DASHBOARD_QUALITY_COL_WIDTHS.length, "TEST - Quality Report");
...
function flushStagedDashboardQualitySectionsRows_(sheet, timing) {
  const keys = Object.keys(ML_DASHBOARD_QUALITY_STAGED_BUFFERS_ || {});
  if (!sheet || !keys.length) return;

  const sectionWrites = [];
  keys.forEach(function(sectionKey) {
    const sectionTitle = getDashboardQualitySectionTitleForKey_(sectionKey);
    if (!sectionTitle) return;
    sectionWrites.push({
      title: sectionTitle,
      rows: buildTimestampedDashboardQualitySectionRows_(sectionKey, ML_DASHBOARD_QUALITY_STAGED_BUFFERS_[sectionKey] || [])
    });
  });

  if (sectionWrites.length) {
    replaceDashboardQualitySectionsRows_(sheet, sectionWrites);
    markFrameworkStep_(timing, "Flushed " + sectionWrites.length + " staged Dashboard Quality sections to sheet canvas");
  }
...
function replaceDashboardQualitySectionsRows_(sheet, sectionWrites) {
  ...
  sectionWrites.slice().sort(function(a, b) {
    ...
    replaceDashboardQualitySectionRows_(sheet, sectionWrite.title, sectionWrite.rows, { sectionBounds: bounds, deferStyle: true, skipBreakApart: true, skipShellCheck: true });
  });
...
function replaceDashboardQualitySectionRows_(sheet, sectionTitle, rows, options) {
  ...
  if (!(options && options.skipShellCheck) && !hasDashboardQualityTemplateShell_(sheet)) {
    rebuildDashboardQualityReportShellCompact_(sheet);
  }
...
function setupSystemSheets() {
  ...
  createIndexSheet();
  forceBaseTemplateHidden_();
  notify_("System sheets setup complete. Use the Organize Tabs menu item when tab sorting is needed.");
  return result;
}
...
function rebuildProductionMonthlyChangeTemplate() {
  ...
  if (template) deleteSheetSafely_(ss, template, "Monthly Change template rebuild");
  ...
  template = insertGovernedOutputSheet_(ss, templateName);
  resizeSheetGrid_(template, 250, lastCol);

  // Clear format block to preserve base plain text structures
  ...
  // Force strict structural text formats on text fields to kill decimal format pollution (0.######).
  applyGovernedTextAndNumberFormats_(template, dashboard, reportHeaders, DATA_START_ROW, 250 - DATA_START_ROW + 1);

  // Re-apply governed subheader layers
...
return Object.freeze({
  installMenu: installMenu_,
  buildTemplates: createOrRefreshAllReportTemplates,
  formatMonthlySheets: formatMonthlySheets,
  createMonthlyStart: runMonthlyStart,
  createMonthlyUpdate: runMonthlyUpdate,
  showTemplates: showTestTemplates_,
  hideTemplates: hideTestTemplates_,
  showMonthlySubReports: showTestMonthlySubReports_,
  hideMonthlySubReports: hideTestMonthlySubReports_,
  showMonthlyActiveReports: showTestMonthlyActiveReports_,
  hideMonthlyActiveReports: hideTestMonthlyActiveReports_,
  hideSourceSheets: hideTestSourceSheets_
});
})();

/** Run once, or call from the production onOpen(), to install the test menu. */
function installNativeBandingTestMenu() { return NativeBandingTest.installMenu(); }
function onOpenNativeBandingTest() { return NativeBandingTest.installMenu(); }
function buildTestTemplatesNativeBanding() { return NativeBandingTest.buildTemplates(); }
function testFormatMonthlySheets() { return NativeBandingTest.formatMonthlySheets(); }
function testCreateMonthlyStart() { return NativeBandingTest.createMonthlyStart(); }
function testCreateMonthlyUpdate() { return NativeBandingTest.createMonthlyUpdate(); }
function showTestTemplates() { return NativeBandingTest.showTemplates(); }
function hideTestTemplates() { return NativeBandingTest.hideTemplates(); }
function showTestMonthlySubReports() { return NativeBandingTest.showMonthlySubReports(); }
function hideTestMonthlySubReports() { return NativeBandingTest.hideMonthlySubReports(); }
function showTestMonthlyActiveReports() { return NativeBandingTest.showMonthlyActiveReports(); }
function hideTestMonthlyActiveReports() { return NativeBandingTest.hideMonthlyActiveReports(); }
function hideTestSourceSheets() { return NativeBandingTest.hideSourceSheets(); }
