...
function applyNativeBanding_(sheet, range, theme) {
  ...
  // Merged header cells can prevent Sheets from allocating a banding range.
  sheet.getRange(HEADER_ROW, startColumn, 1, cols).breakApart();
...
function testCreateOutputSheetByCopying_(sheetType, outputName, outputRows, firstDay, endDate, timing, timingLabel) {
  ...
  sheet.getRange(HEADER_ROW, 1, 1, width).breakApart().setValues([templateHeaders]);
  ...
  if (getBehaviorForSheetType_(dashboard, normalizedSheetType).usesAlternatingColors) {
    const theme = getThemeColorsFromBase_(sheetDef.baseColor, dashboard.globals || RFF_DEFAULTS);
    applyNativeBanding_(sheet, sheet.getRange(DATA_START_ROW, 1, Math.max(rows.length, 1), width), theme);
  }
