/**
 * Master List / Report Formatter Framework v1.8.9.8.4
 *
 * Wave 4 corrections release notes:
 * - Consolidates hidden 100-row archetype builds under buildAllTemplatesAndValidate.
 * - Routes B, CD, UC, and RD through one untouched-source template-copy pipeline.
 * - Uses template-inherited native banding with no runtime alternating-color repaint.
 * - Standardizes multi-section surfaces on the governed five-row sub-header tuple.
 * - Routes Refined Data, Disenrolled, Monthly Change, Master List, and Index through
 *   canonical production entry points while retaining compatibility aliases.
 * - Adds early disenrollment filtering and primitive row-hash skipping for changes.
 * - Removes signature-cache and superseded formatter/paint implementations.
 */
...
const MASTER_LIST_MERGE_ML_VERSION = "1.8.9.8.4";
...
function getFormatDashboardExpectedSectionHeaders_() {
  const width = 19; // Expanded width to fit pre-calculated HSL Level 1-5 Hex columns
  const map = {};
  ...
  // UPDATED SECTION C HEADERS (Includes Level 1-5 Hex columns)
  map[RFF_SECTION_SHEETS] = normalizeSectionRowForWidth_(["Sheet Type", "Report Title", "Template Name", "Output Naming Pattern", "Base Color", "Level 1 Hex", "Level 2 Hex", "Level 3 Hex", "Level 4 Hex", "Level 5 Hex", "Use Prompt Date", "End Date Source", "Template Row Count", "Template Column Count", "Template Row Mode", "Minimum Rows", "Buffer Rows"], width);
  ...
  // UPDATED SECTION E HEADERS (Includes Level 1-5 Hex columns)
  map[RFF_SECTION_SYSTEM_SURFACES] = normalizeSectionRowForWidth_(["System Sheet Name", "Display Name", "Sort Order", "Uses Title Rows", "Uses Filter", "Uses Alternating Colors", "Uses Subheaders", "Hidden Template", "Output Visibility", "Default Column Widths", "Base Color", "Level 1 Hex", "Level 2 Hex", "Level 3 Hex", "Level 4 Hex", "Level 5 Hex", "Title Font Color", "Notes"], width);
...
function writeDashboardDefaultsFast_(sheet) {
  const width = 19; // Expanded width to fit 19-column Section C and Section E matrices
  const rows = [];
  ...
  const widths = [220, 160, 220, 180, 110, 100, 100, 100, 100, 100, 120, 160, 140, 140, 120, 120, 120, 120, 120];
...
function normalizeToDateObject_(value) {
  ...
  if (delimiter) {
    ...
    if (parts.length === 2) {
      ...
      if (monthDate.getFullYear() === year && monthDate.getMonth() === month - 1 && isReasonableReportDate_(monthDate)) return monthDate;
    }
    if (parts.length === 3) {
      ...
      if (parsed.getFullYear() === year && parsed.getMonth() === month - 1 && parsed.getDate() === day && isReasonableReportDate_(parsed)) return parsed;
    }
  }
...
function trimOutputSheetToDataSize_(sheet, rowCount, sheetType) {
  ...
  const templateRows = RFF_TEMPLATE_BASELINE_ROWS;
...
function getThemeColorsFromBase_(hex, globals, contextDef) {
  // 1. FAST PATH: Pull directly from pre-calculated dashboard hex columns if available
  if (contextDef && contextDef.level1 && contextDef.level5) {
    return {
      base: contextDef.baseColor || normalizeHex_(hex),
      level1: normalizeHex_(contextDef.level1),
      level2: normalizeHex_(contextDef.level2),
      level3: normalizeHex_(contextDef.level3),
      level4: normalizeHex_(contextDef.level4), // 90% Lightness
      level5: normalizeHex_(contextDef.level5)  // 97% Lightness
    };
  }

  // 2. SECONDARY CACHE: Return cached palette if previously computed in memory
  globals = globals || {};
  const baseHex = normalizeHex_(hex);
  const cache = getRuntimeCache_().themeColors;
  if (cache[baseHex]) return cache[baseHex];

  // 3. FALLBACK: Default HSL lightness levels (Level 4 = 90%, Level 5 = 97%)
  const levels = [
    numberOrDefault_(globals.hslLevel1, 60), // Level 1: Tab Accents / Visual Spacers
    numberOrDefault_(globals.hslLevel2, 75), // Level 2: Column Headers
    numberOrDefault_(globals.hslLevel3, 85), // Level 3: Section Title Bars
    numberOrDefault_(globals.hslLevel4, 90), // Level 4: Alternating Row Color 1 (90%)
    numberOrDefault_(globals.hslLevel5, 97)  // Level 5: Alternating Row Color 2 (97%)
  ];
...
function rebuildFrameworkTimingReportShellCompact_(sheet) {
  ...
  applyFrameworkTimingSurfaceFromDashboard_(sheet);
}
...
function applyFrameworkTimingSurfaceFromDashboard_(sheet, sectionIds) {
  if (!sheet) return;
...
function replaceFrameworkTimingSectionRows_(sheet, sectionId, rows, options) {
  ...
  if (!options || !options.skipStyle) applyFrameworkTimingSurfaceFromDashboard_(sheet, [section.id]);
}
...
function applySubHeaderBlock_(sheet, startRow, sectionTitle, lastUpdatedText, headers, dashboard, sheetDef, options) {
  ...
  // Pass activeSheetDef so pre-calculated Level 1-5 Hex codes are pulled directly
  const theme = getThemeColorsFromBase_(activeSheetDef.baseColor || "#A165CC", activeDashboard.globals || RFF_DEFAULTS, activeSheetDef);
  ...
  // Row 1: Buffer Row (25px)
  if (!valuesAlreadyWritten) sheet.getRange(startRow, 1, 1, width).clearContent().clearFormat();
  safeSetRowHeights_(sheet, startRow, 1, 25, "Five-row top buffer");

  // Row 2: Section Title (14pt Bold, Level 3 Fill, 25px) + Col C "Last Updated" (10pt Italic)
  const titleRange = sheet.getRange(startRow + 1, 1, 1, width);
  titleRange.breakApart();
  if (!valuesAlreadyWritten) {
    sheet.getRange(startRow + 1, 1).setValue(sectionTitle).setFontSize(14).setFontWeight("bold");
    sheet.getRange(startRow + 1, 3).setValue("Last Updated - " + updated).setFontSize(10).setFontStyle("italic");
  }
  sheet.getRange(startRow + 1, 1, 1, width)
    ...
    .setVerticalAlignment("middle")
    .setWrapStrategy(SpreadsheetApp.WrapStrategy.OVERFLOW);
  safeSetRowHeights_(sheet, startRow + 1, 1, 25, "Five-row section title");

  // Row 3: Visual Spacer Row (10px, Level 1 Fill)
  const spacerRange = sheet.getRange(startRow + 2, 1, 1, width).breakApart();
  if (!valuesAlreadyWritten) spacerRange.clearContent();
  spacerRange.clearFormat().setBackground(theme.level1);
  safeSetRowHeights_(sheet, startRow + 2, 1, 10, "Five-row visual spacer");

  // Row 4: Column Headers (8pt Bold, Level 2 Fill, Default 25px with Auto-Wrap Expand)
  ...
  headerRange
    ...
    .setWrapStrategy(SpreadsheetApp.WrapStrategy.WRAP)
    .setVerticalAlignment("top");
  safeSetRowHeights_(sheet, startRow + 3, 1, 25, "Five-row column headers");

  // Row 5: Data Anchor Row (25px)
  ...
  safeSetRowHeights_(sheet, startRow + 4, 1, 25, "Five-row data anchor");
  ...
  return {
    ...
    dataAnchorRow: startRow + 4
  };
}


function replaceSectionDataInTemplate_(sheet, sectionTitle, dataRows, headers) {
  if (!sheet) throw new Error("Section template sheet is required.");
  const width = Math.max((headers || []).length, (dataRows && dataRows[0] ? dataRows[0].length : 1));
  const lastRow = Math.max(sheet.getLastRow(), 1);
  const values = sheet.getRange(1, 1, lastRow, 1).getDisplayValues();
  let titleRow = 0;
  for (let i = 0; i < values.length; i++) {
    if (String(values[i][0] || "").indexOf(sectionTitle) === 0) { titleRow = i + 1; break; }
  }
  if (!titleRow) throw new Error("Section title not found: " + sectionTitle);
  const startRow = titleRow - 1;
  const dataAnchorRow = startRow + 4;
  const rows = (dataRows || []).map(function(row) {
    const out = new Array(width).fill("");
    for (let col = 0; col < width; col++) out[col] = row && row[col] !== undefined ? row[col] : "";
    return out;
  });
  const requiredRows = Math.max(rows.length, 1);
  const requiredEndRow = dataAnchorRow + requiredRows - 1;
  if (sheet.getMaxRows() < requiredEndRow) {
    sheet.insertRowsAfter(sheet.getMaxRows(), requiredEndRow - sheet.getMaxRows());
  }
  const target = sheet.getRange(dataAnchorRow, 1, requiredRows, width);
  target.clearContent().clearFormat();
  if (rows.length) target.setValues(rows);
  return rows.length;
}
...
function updateBaseTemplateCanvas_(ss, sheetType, dashboard) {
  ...
  template.showColumns(1, template.getMaxColumns());
  template.showSheet();
  template.getRange(HEADER_ROW, 1, 1, width)
    ...
    .setFontColor("#000000");
  template.setRowHeight(HEADER_ROW, 35);
  ...
  safeSetRowHeights_(template, DATA_START_ROW, dataRows, 21, "Base template data grid");
  template.setFrozenRows(HEADER_ROW);
  if (template.getMaxColumns() > width) template.deleteColumns(width + 1, template.getMaxColumns() - width);
  clearSheetRuntimeCachesForSheet_(template);
  return template;
}
...
function rebuildTemplateCanvas_(dashboard, sheetDef, timing) {
  const sheet = createOrRefreshTemplateFromDashboard_(dashboard, sheetDef, timing);
  if (sheet && sheet.getMaxColumns() > 0) sheet.showColumns(1, sheet.getMaxColumns());
  return sheet;
}
...
function fuseMasterListRowsInMemory_(rows, masterHeaderMap, sourceMaps) {
  const output = rows || [];
  (sourceMaps || []).forEach(function(sourceMap) {
    if (!sourceMap || typeof sourceMap.get !== "function") return;
    output.forEach(function(row) {
      const pmrIndex = getPMRIndex_(masterHeaderMap || {});
      const source = pmrIndex >= 0 ? sourceMap.get(normalizePMR_(row[pmrIndex])) : null;
      if (!source) return;
      Object.keys(source).forEach(function(header) {
        const index = masterHeaderMap[header];
        if (index !== undefined && (row[index] === "" || row[index] === null)) row[index] = source[header];
      });
    });
  });
  return output;
...
function flushDashboardQualityBufferToGrid_(sheet, startRow, rows) {
...
function flushFrameworkTimingBufferToGrid_(sheet, startRow, rows) {
  if (!sheet || !rows || !rows.length) return 0;
  sheet.insertRowsBefore(startRow, rows.length);
  sheet.getRange(startRow, 1, rows.length, rows[0].length).setValues(rows);
  return rows.length;
...
// Public compatibility aliases route every trigger to one canonical engine.
function buildDemoPFromScratch() { return buildRefinedDataFromScratch(); }
function updateDemoPMonthlySync() { return updateRefinedDataMonthlySync(); }
function buildDisenrolledExclusionReport() { return buildDisenrolledExclusionFromRefinedData(); }
function createDisenrolledList() { return buildDisenrolledExclusionFromRefinedData(); }
function buildMonthlyChangeReport() { return buildMonthlyChangeFromSources(); }
function buildMasterListReport() { return buildMasterListFromRefinedData(); }
function createMasterList() { return buildMasterListFromRefinedData(); }
...
function getDefaultSheetDefinitionRowsWithColumnCounts_() {
  const counts = {};
  const globals = loadGlobalSettings_();
  ...
  return getDefaultSheetDefinitionRows_().map(function(row) {
    ...
    // Injects pre-calculated Level 1-5 Hex codes right after Base Color (index 4)
    return [
      row[0], // Sheet Type
      row[1], // Report Title
      row[2], // Template Name
      ...
      counts[sheetType] || "", // Template Column Count
      row[8], // Template Row Mode
      row[9], // Minimum Rows
      row[10] // Buffer Rows
    ];
  });
...
function getDefaultSystemSurfaceRows_() {
  ...
  return baseSystemRows.map(function(row) {
    const baseColor = normalizeHex_(row[10] || RFF_SYSTEM_SHEET_TITLE_COLOR);
    const theme = getThemeColorsFromBase_(baseColor, globals);

    // Injects pre-calculated Level 1-5 Hex codes right after Base Color (index 10)
    return [
      row[0],  // System Sheet Name
      row[1],  // Display Name
      row[2],  // Sort Order
      row[3],  // Uses Title Rows
      row[4],  // Uses Filter
      row[5],  // Uses Alternating Colors
      row[6],  // Uses Subheaders
      row[7],  // Hidden Template
      row[8],  // Output Visibility
      row[9],  // Default Column Widths
      baseColor,
      theme.level1,
      theme.level2,
      theme.level3,
...
function buildAllTemplatesAndValidate() {
  const buildResult = runFrameworkTimed_("Build All Templates And Validate", function(timing) {
    ...
    // Explicitly verify/build Golden Master base template alongside report templates
    ensureGoldenMasterTemplate_(dashboard, timing);
    ...
    sortSheetDefinitionsByProductionOrder_(dashboard.sheetDefinitions).forEach(function(sheetDef) {
      try {
        const baselineDef = Object.assign({}, sheetDef, {
          templateRowCount: 100,
          minimumRows: 100,
          bufferRows: 0,
          templateRowMode: "FIXED"
        });
        const template = createOrRefreshTemplateFromDashboard_(dashboard, baselineDef, timing);
        template.showColumns(1, template.getMaxColumns());
...
function loadSystemSurfaces_(sheet) {
  ...
  (rows || []).forEach(function(row) {
    ...
    surfaces[name] = {
      ...
      defaultColumnWidths: String(row[9] || "").trim(),
      baseColor: normalizeHex_(row[10] || RFF_SYSTEM_SHEET_TITLE_COLOR),
      // Direct pre-calculated HSL pulls
      level1: normalizeHex_(row[11] || "#B3D7E8"),
      level2: normalizeHex_(row[12] || "#C8E3F0"),
      level3: normalizeHex_(row[13] || "#DCEEF6"),
      level4: normalizeHex_(row[14] || "#F5FAFC"),
      level5: normalizeHex_(row[15] || "#FAFDFF"),
      titleFontColor: normalizeHex_(row[16] || "#000000"),
  ...
  return rows
    ...
    .map(function(row) {
      return {
        ...
        outputNamingPattern: String(row[3] || "").trim(),
        baseColor: normalizeHex_(row[4] || "#65A9CC"),
        // Direct pre-calculated HSL pulls (no runtime math needed)
        level1: normalizeHex_(row[5] || "#B3D7E8"),
        level2: normalizeHex_(row[6] || "#C8E3F0"),
        level3: normalizeHex_(row[7] || "#DCEEF6"),
        level4: normalizeHex_(row[8] || "#F5FAFC"),
        level5: normalizeHex_(row[9] || "#FAFDFF"),
        usePromptDate: parseBoolean_(row[10]),
        endDateSource: String(row[11] || "").trim(),
        templateRowCount: numberOrDefault_(row[12], RFF_DEFAULTS.templateRows),
        templateColumnCount: numberOrDefault_(row[13], 0),
        templateRowMode: String(row[14] || "FIXED").trim().toUpperCase(),
        minimumRows: numberOrDefault_(row[15], RFF_DEFAULTS.templateRows),
        bufferRows: numberOrDefault_(row[16], 100)
      };
    });
}
...
function promoteStagedTemplateBuild_(ss, buildSheet, existingSheet, sheetDef, timing) {
  ...
  if (typeof buildSheet.showSheet === "function") {
    showSheetIfNeeded_(buildSheet, timing, "Show promoted template: " + sheetDef.templateName);
  }
}
...
function applyStandardTitleRowsForSystemReport_(sheet, reportTitle, versionText, colCount, dashboard) {
  const activeDashboard = dashboard || loadDashboardConfig_();
  const globals = activeDashboard.globals || RFF_DEFAULTS;
  const width = Math.max(colCount || 1, 4);

  // Row 1: Title (A1) & Version (B1), Column C cleared
  sheet.getRange(1, 1).setValue(reportTitle).setFontSize(globals.titleFontSize || 14).setFontWeight("bold");
  sheet.getRange(1, 2).setValue(versionText || ("- v" + RFF_VERSION + " -")).setFontSize(10);
  sheet.getRange(1, 3).setValue(""); // Clear C1
  safeSetRowHeights_(sheet, 1, 1, globals.row1Height || 25, "Title Row 1");

  // Row 2: Date Range
  sheet.getRange(2, 1).setValue("Date");
  sheet.getRange(2, 2).setValue(new Date()).setNumberFormat(globals.defaultDateFormat || "mm/dd/yyyy");
  sheet.getRange(2, 3).setValue("to").setHorizontalAlignment("center");
  sheet.getRange(2, 4).setValue(new Date()).setNumberFormat(globals.defaultDateFormat || "mm/dd/yyyy");
  safeSetRowHeights_(sheet, 2, 1, globals.row2Height || 20, "Title Row 2");

  // Row 3: Visual Spacer Row
  sheet.getRange(3, 1, 1, width).setBackground("#FFFFFF").clearContent();
  safeSetRowHeights_(sheet, 3, 1, globals.row3Height || 10, "Title Row 3");

  // Row 4: Header Row Left Blank for Diagnostic Telemetry Sheets
  if (reportTitle === "Framework Timing Report" || reportTitle === "Dashboard Quality Report") {
    sheet.getRange(4, 1, 1, width).clearContent().setBackground("#FFFFFF");
  }
  safeSetRowHeights_(sheet, 4, 1, globals.headerRowHeight || 40, "Title Row 4");
}
...
function applySystemSurfaceWidths_(sheet, widthSpec, colCount) {
  if (!sheet || !colCount) return;
  const spec = String(widthSpec || "").trim().toUpperCase();

  if (spec === "FIT_TO_DATA" || spec === "AUTO") {
  ...
  const parsed = spec.split(",").map(function(val) { return Number(val.trim()); }).filter(function(val) { return isFinite(val) && val > 0; });

  if (parsed.length === 1) {
    // Single uniform width applied across all columns
    sheet.setColumnWidths(1, colCount, parsed[0]);
  } else if (parsed.length > 1) {
    // Comma-separated column width array
    for (let i = 0; i < Math.min(parsed.length, colCount); i++) {
      sheet.setColumnWidth(i + 1, parsed[i]);
    }
  } else {
    // Fallback default width
    sheet.setColumnWidths(1, colCount, RFF_DEFAULTS.defaultColumnWidth || 105);
  }
}
...
function applyNativeBandingToRange_(sheet, range, theme) {
  ...
  // Unmerge header row across target column span before applying banding
  sheet.getRange(HEADER_ROW, startColumn, 1, columns).breakApart();

  // Clear existing bandings that overlap with target range to prevent stacking
  ...
  // Apply native Google Sheets row banding
  const banding = sheet.getRange(startRow, startColumn, rows, columns)
    .applyRowBanding(SpreadsheetApp.BandingTheme.LIGHT_GREY, false, false);

  // Alternating colors set to Level 4 (90% lightness) and Level 5 (97% lightness)
  banding.setFirstRowColor((theme && theme.level4) || "#E6E6E6");  // Level 4 (90%)
  banding.setSecondRowColor((theme && theme.level5) || "#F7F7F7"); // Level 5 (97%)
...
function getUntouchedSourceDataForTemplate_(sourceSheet, targetHeaders) {
  ...
  const values = dataStart <= lastRow
    ? sourceSheet.getRange(dataStart, 1, lastRow - dataStart + 1, lastColumn).getValues()
    : [];
...
function formatMonthlySubReportViaTemplate_(sourceSheet, sheetType, monthParts, timing, options) {
  ...
  if (titleInfo) {
    outputSheet.getRange("C1:D1").merge().setValue(titleInfo);
    if (normalizedType === SHEET_TYPE.CARE_PLAN_DUE || normalizedType === SHEET_TYPE.UNLOCKED) {
      outputSheet.getRange("C1").setFontSize(5).setWrapStrategy(SpreadsheetApp.WrapStrategy.WRAP);
    }
  }
  ...
  if (normalizedType !== SHEET_TYPE.RAW_DATA) applyColumnHidingFromDashboard_(outputSheet, context.dashboard, headers);
...
function processDemoP() { return buildRefinedDataFromScratch(); }
...
function createActiveDemoPFromRawData_(rawSheet, targetName, monthParts, timing) {
  ...
  const demoSheet = createOutputSheetFromDashboardTemplate_(
    ...
    monthParts.lastDay,
    timing,
    "Refined Data unified template write"
  );
...
function buildMonthlyChangeFromSources() {
...
function createDisenrolledListForMonth_(monthParts, timing, options) {
  ...
  const sourceIsEmpty = !demoSheet || demoSheet.getLastRow() < DATA_START_ROW;
  if (sourceIsEmpty) {
    const emptySheet = createDisenrolledExclusionSheetFromDashboardTemplate_(ss, monthParts);
    const lastRow = emptySheet.getLastRow();
    if (lastRow >= DATA_START_ROW) emptySheet.getRange(DATA_START_ROW, 1, lastRow - DATA_START_ROW + 1, Math.max(emptySheet.getLastColumn(), 1)).clearContent();
    updateDisenrolledExclusionReportDates_(emptySheet, monthParts);
    const emptyResult = { pmrsRemoved: 0, rowsRemoved: 0, rowsCopied: 0 };
    setLastRawDataDisenrolledBuildResult_(emptyResult);
    notify_("Refined Data is missing or empty. Disenrolled Exclusion was left as a clean, empty governed canvas.");
    step("Stopped safely - Refined Data missing or empty");
    return emptyResult;
  }
  step("Locate Refined Data: " + demoSheet.getName());
...
function buildMasterListFromRefinedData() {
...
function createMasterListSheetFromTemplate_(ss, targetName, monthParts, timing, timingLabel) {
  return createOutputSheetFromDashboardTemplate_(
    ...
    monthParts.lastDay,
    timing,
    timingLabel || "Master List template copy"
  );
}
...
function getMonthPartsFromTitleRows_(sheet) {
  const firstDay = normalizeToDateObject_(sheet.getRange("B2").getValue()) ||
    normalizeToDateObject_(sheet.getRange("B2").getDisplayValue());
  const lastDay = normalizeToDateObject_(sheet.getRange("D2").getValue()) ||
    normalizeToDateObject_(sheet.getRange("D2").getDisplayValue());
...
function buildDisenrolledExclusionFromRefinedData() {
...
function moveDisenrolledPMRsFromDemoPToExclusion_(demoSheet, monthParts, timing) {
  ...
  step("Demo P alternating colors reapplied after retained rewrite");
...
function writeDemoPDisenrollmentRetainedBody_(demoSheet, retainedRows, width, timing) {
  ...
  if (normalizedRows.length) {
    demoSheet.getRange(DATA_START_ROW, 1, normalizedRows.length, width).setValues(normalizedRows);
    step("retained Demo P alternating colors reapplied");
    step("retained Demo P body written | Rows: " + normalizedRows.length);
  } else {
    step("retained Demo P body write skipped - no rows");
  }
...
function stampDisenrollmentReportDateInMemory_(rows, headerMap, reportDate) {
  const addedIdx = headerMap ? headerMap[DISENROLLED_EXCLUSION_ADDED_HEADER] : undefined;
  if (addedIdx === undefined) return rows || [];
  const nativeDate = reportDate instanceof Date ? new Date(reportDate.getTime()) : getTodayLocalDate_();
  (rows || []).forEach(function(row) {
    if (row[addedIdx] === "" || row[addedIdx] === null || row[addedIdx] === undefined) row[addedIdx] = new Date(nativeDate.getTime());
  });
  return rows || [];
}
...
function appendDisenrolledDeltasToExclusionSheet_(exclusionSheet, sourceHeaders, rowsToAppendByPMR, monthParts, timing) {
  ...
  stampDisenrollmentReportDateInMemory_(appendPayload, exclusionHeaderMap, monthParts && monthParts.firstDay ? monthParts.firstDay : getTodayLocalDate_());
  step("append payload built and first-seen dates stamped | Rows: " + appendPayload.length);
  ...
  if (addedIdx !== undefined) exclusionSheet.getRange(insertStartRow, addedIdx + 1, appendPayload.length, 1).setNumberFormat("mm/dd/yyyy");
  step("append payload values written without format brush | Rows: " + appendPayload.length + "; Width: " + width);
...
function ensureDashboardQualitySheetShellForWorkflow_(sheet, dashboard, timing) {
  ...
  if (!hasDashboardQualityTemplateShell_(sheet)) {
    ...
    markFrameworkStep_(timing, "Dashboard Quality matrix shell structure verified; layout rebuild bypassed");
...
function applySystemSurfaceStructure_(sheet, surface) {
  if (!sheet || !surface) return sheet;
  const widths = surface.defaultColumnWidths && surface.defaultColumnWidths.length ? surface.defaultColumnWidths : [RFF_DEFAULTS.defaultColumnWidth];
  if (sheet.getMaxRows() < RFF_TEMPLATE_BASELINE_ROWS) sheet.insertRowsAfter(sheet.getMaxRows(), RFF_TEMPLATE_BASELINE_ROWS - sheet.getMaxRows());
  if (sheet.getMaxColumns() < widths.length) sheet.insertColumnsAfter(sheet.getMaxColumns(), widths.length - sheet.getMaxColumns());
  applyColumnWidthsInRuns_(sheet, widths);
  if (surface.usesTitleRows) {
    if (!String(sheet.getRange("A1").getDisplayValue() || "").trim()) sheet.getRange("A1").setValue(surface.displayName || surface.systemSheetName);
    sheet.getRange(1, 1, 1, Math.max(widths.length, 1)).setBackground(surface.titleFillColor).setFontColor(surface.titleFontColor).setFontWeight("bold");
  }
  if (surface.usesFilter && sheet.getLastRow() >= HEADER_ROW && sheet.getLastColumn() > 0 && !sheet.getFilter()) {
    sheet.getRange(HEADER_ROW, 1, Math.max(sheet.getLastRow() - HEADER_ROW + 1, 1), sheet.getLastColumn()).createFilter();
  }
  if (surface.usesAlternatingColors && sheet.getLastRow() >= DATA_START_ROW) {
    const bandingRange = sheet.getRange(DATA_START_ROW, 1, Math.max(sheet.getLastRow() - DATA_START_ROW + 1, 1), Math.max(sheet.getLastColumn(), 1));
    applyNativeBandingToRange_(sheet, bandingRange, { level3: surface.titleFillColor, level4: "#F7F7F7" });
  }
...
function buildSystemSheetsCore_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const surfaces = getSystemSurfaceConfigMap_();
  const built = {};
  Object.keys(surfaces).sort(function(a, b) { return surfaces[a].sortOrder - surfaces[b].sortOrder; }).forEach(function(name) {
    if (name === RFF_BASE_TEMPLATE_NAME) return;
    let sheet = ss.getSheetByName(name);
    if (!sheet) sheet = insertGovernedOutputSheet_(ss, name);
    applySystemSurfaceStructure_(sheet, surfaces[name]);
    placeCreatedSheetInConfiguredOrder_(sheet);
    built[name] = sheet;
  });
  return {
    sheets: built,
    dashboardConfigSheet: built[RFF_DASHBOARD_SHEET] || ss.getSheetByName(RFF_DASHBOARD_SHEET),
    frameworkTiming: built[RFF_TIMING_SHEET] || ss.getSheetByName(RFF_TIMING_SHEET),
    dashboardQuality: built[RFF_TEST_DASHBOARD_SHEET] || ss.getSheetByName(RFF_TEST_DASHBOARD_SHEET),
    index: built.Index || ss.getSheetByName("Index"),
    demoPArchive: built[DEMO_P_ARCHIVE_SHEET] || ss.getSheetByName(DEMO_P_ARCHIVE_SHEET)
  };
}
...
function buildSystemSheets() {
  ...
  if (typeof notify_ === "function") notify_("System sheets physically built based on Section E configuration.");
}

function setupSystemSheets() {
  // Step 1: Rebuild Format Dashboard defaults FIRST so Section E configurations exist on grid
  rebuildFormatDashboardDefaults();
  
  // Step 2: Build physical system sheets using the newly painted Section E surfaces
  buildSystemSheets();
  
  // Step 3: Initialize timing and start-up quality reports
  ensureFrameworkTimingReport_();
  runDashboardQualityStartUp();
  
  if (typeof notify_ === "function") notify_("System sheets setup and initialized.");
}

function initializeSystemSheets_() {
  const result = buildSystemSheetsCore_();
  const qualitySheet = result.dashboardQuality;
  const timingSheet = result.frameworkTiming;
  if (qualitySheet) {
    if (!hasDashboardQualityTemplateShell_(qualitySheet)) rebuildDashboardQualityReportShellCompact_(qualitySheet);
    else applyDashboardQualityReportColumnSettings_(qualitySheet, { skipRangeFormat: true });
  }
  if (timingSheet) {
    if (!hasFrameworkTimingReportShell_(timingSheet)) rebuildFrameworkTimingReportShellCompact_(timingSheet);
  }
  return result;
}
...
function flushStagedDashboardQualitySectionsRows_(sheet, timing) {
  if (!sheet) return;
  const width = RFF_DASHBOARD_QUALITY_COL_WIDTHS.length;
  const masterQualityBuffer = [normalizeSectionRowForWidth_(["Dashboard Quality Report", "- v" + MASTER_LIST_MERGE_ML_VERSION + " -", "Report Formatter Framework"], width)];
  const sectionStarts = [];
  const updated = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "MM/dd/yyyy");
  RFF_DASHBOARD_QUALITY_SECTIONS.forEach(function(section) {
    ...
    const header = stored.length ? normalizeSectionRowForWidth_(stored[0], width) : normalizeSectionRowForWidth_(getDefaultDashboardQualityDetailHeader_(section), width);
    const details = stored.length > 1 ? stored.slice(1) : [["NOT RUN", getDashboardQualityNotRunMessage_(section)]];
    sectionStarts.push({ startRow: masterQualityBuffer.length + 1, section: section, header: header });
    masterQualityBuffer.push(normalizeSectionRowForWidth_([], width));
    masterQualityBuffer.push(normalizeSectionRowForWidth_([section.title + " | Last Updated - " + updated], width));
    masterQualityBuffer.push(normalizeSectionRowForWidth_([], width));
    masterQualityBuffer.push(header);
    masterQualityBuffer.push(normalizeSectionRowForWidth_([], width));
    details.forEach(function(row) { masterQualityBuffer.push(normalizeSectionRowForWidth_(row, width)); });
  });
  if (sheet.getMaxRows() < masterQualityBuffer.length) sheet.insertRowsAfter(sheet.getMaxRows(), masterQualityBuffer.length - sheet.getMaxRows());
  const clearRows = Math.max(sheet.getLastRow(), masterQualityBuffer.length, 1);
  sheet.getRange(1, 1, clearRows, width).clearContent().breakApart();
  sheet.getRange(1, 1, masterQualityBuffer.length, width).setValues(masterQualityBuffer);
  const dashboard = loadDashboardConfig_();
  const sheetDef = getSheetDefinitionByTypeOrNull_(dashboard, "Dashboard Quality Report") || { baseColor: "#79b5d2" };
  sectionStarts.forEach(function(item) {
    applySubHeaderBlock_(sheet, item.startRow, item.section.title, updated, item.header, dashboard, sheetDef, { valuesAlreadyWritten: true });
  });
  if (timing) markFrameworkStep_(timing, "Dashboard Quality master buffer flushed in one values write | Sections: " + sectionStarts.length);
...
function setupSystemSheets() {
  return runFrameworkTimed_("Set up System Sheets", function(timing) {
    const existingDashboard = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(RFF_DASHBOARD_SHEET);
    if (migrateSystemSurfaceSectionToExpandedSchema_(existingDashboard, timing)) clearDashboardConfigCache_();
    let result = buildSystemSheetsCore_();
    const dashboardSheet = result.dashboardConfigSheet;
    if (dashboardSheet && dashboardSheet.getLastRow() <= 1) {
      writeDashboardDefaultsFast_(dashboardSheet);
      clearDashboardConfigCache_();
      result = buildSystemSheetsCore_();
      markFrameworkStep_(timing, "Format Dashboard defaults populated before Section E rebuild");
    }
    if (result.dashboardQuality && !hasDashboardQualityTemplateShell_(result.dashboardQuality)) rebuildDashboardQualityReportShellCompact_(result.dashboardQuality);
    if (result.frameworkTiming && !hasFrameworkTimingReportShell_(result.frameworkTiming)) rebuildFrameworkTimingReportShellCompact_(result.frameworkTiming);
    updateIndexSheet({ forceShellRebuild: true });
    enforceGlobalSheetSortOrder_(getMonthDateParts_(new Date()));
    markFrameworkStep_(timing, "System sheets set up, verified, and ordered without creating RFF_BASE_TEMPLATE");
    notify_("System sheets setup complete. Run Build All Templates + Validate to create RFF_BASE_TEMPLATE and report templates.");
    return result;
  });
}
...
function rebuildProductionMonthlyChangeTemplate() {
  ...
  // Section subheaders are injected dynamically by applySubHeaderBlock_ at runtime.
